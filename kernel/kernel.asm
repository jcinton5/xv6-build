...
main()
{
  ...
  scheduler();        
    ...
    binit();         // buffer cache
    80000eca:	4e3010ef          	jal	80002bac <binit>
    iinit();         // inode table
    80000ece:	268020ef          	jal	80003136 <iinit>
    fileinit();      // file table
    80000ed2:	15a030ef          	jal	8000402c <fileinit>
    virtio_disk_init(); // emulated hard disk
...
forkret(void)
{
  ...
  if (first) {
    ...
    8000191e:	4d5010ef          	jal	800035f2 <fsinit>
...
0000000080001b94 <userinit>:
{
  ...
  p->cwd = namei("/");
    ...
    80001bb4:	761010ef          	jal	80003b14 <namei>
...
0000000080001c32 <kfork>:
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  acquire(&wait_lock);
    ...
    80001d02:	c8248493          	addi	s1,s1,-894 # 8000f980 <wait_lock>
...
0000000080001d40 <scheduler>:
{
  ...
  c->proc = 0;
    ...
    80001d64:	c0870713          	addi	a4,a4,-1016 # 8000f968 <pid_lock>
    80001d68:	975a                	add	a4,a4,s6
    80001d6a:	02073823          	sd	zero,48(a4)
        swtch(&c->context, &p->context);
    80001d6e:	0000e717          	auipc	a4,0xe
    80001d72:	c3270713          	addi	a4,a4,-974 # 8000f9a0 <cpus+0x8>
    ...
    80001d80:	beca0a13          	addi	s4,s4,-1044 # 8000f968 <pid_lock>
    80001d84:	9a3e                	add	s4,s4,a5
        found = 1;
    80001d86:	4b85                	li	s7,1
    for(p = proc; p < &proc[NPROC]; p++) {
    80001d88:	00014997          	auipc	s3,0x14
    80001d8c:	a1098993          	addi	s3,s3,-1520 # 80015798 <tickslock>
  ...
  asm volatile("csrw sstatus, %0" : : "r" (x));
    ...
    80001dea:	fb248493          	addi	s1,s1,-78 # 8000fd98 <proc>
      if(p->state == RUNNABLE) {
...
0000000080001df2 <sched>:
{
  ...
  if(mycpu()->noff != 1)
    ...
    80001e16:	b5670713          	addi	a4,a4,-1194 # 8000f968 <pid_lock>
  ...
  intena = mycpu()->intena;
    ...
    80001e3c:	b3090913          	addi	s2,s2,-1232 # 8000f968 <pid_lock>
  ...
  swtch(&p->context, &mycpu()->context);
    ...
    80001e54:	b5058593          	addi	a1,a1,-1200 # 8000f9a0 <cpus+0x8>
...
wakeup(void *chan)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001f3c:	e6048493          	addi	s1,s1,-416 # 8000fd98 <proc>
    if(p != myproc()){
      acquire(&p->lock);
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001f48:	85490913          	addi	s2,s2,-1964 # 80015798 <tickslock>
...
0000000080001f8e <reparent>:
{
  ...
  for(pp = proc; pp < &proc[NPROC]; pp++){
    ...
    80001fa4:	df848493          	addi	s1,s1,-520 # 8000fd98 <proc>
      pp->parent = initproc;
    80001fa8:	00006a17          	auipc	s4,0x6
    80001fac:	8b8a0a13          	addi	s4,s4,-1864 # 80007860 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001fb0:	00013997          	auipc	s3,0x13
    80001fb4:	7e898993          	addi	s3,s3,2024 # 80015798 <tickslock>
...
0000000080001fe4 <kexit>:
{
  ...
  if(p == initproc)
    ...
    80002000:	8647b783          	ld	a5,-1948(a5) # 80007860 <initproc>
    ...
    80002018:	fc8fe0ef          	jal	800007e0 <panic>
      fileclose(f);
    8000201c:	0d8020ef          	jal	800040f4 <fileclose>
      p->ofile[fd] = 0;
  ...
  begin_op();
    80002030:	4b9010ef          	jal	80003ce8 <begin_op>
  iput(p->cwd);
    80002034:	1509b503          	ld	a0,336(s3)
    80002038:	448010ef          	jal	80003480 <iput>
  end_op();
    8000203c:	517010ef          	jal	80003d52 <end_op>
  p->cwd = 0;
    80002040:	1409b823          	sd	zero,336(s3)
  acquire(&wait_lock);
    80002044:	0000e497          	auipc	s1,0xe
    80002048:	93c48493          	addi	s1,s1,-1732 # 8000f980 <wait_lock>
...
kkill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    8000209a:	d0248493          	addi	s1,s1,-766 # 8000fd98 <proc>
    8000209e:	00013997          	auipc	s3,0x13
    800020a2:	6fa98993          	addi	s3,s3,1786 # 80015798 <tickslock>
    acquire(&p->lock);
...
000000008000213a <kwait>:
{
  ...
  acquire(&wait_lock);
    ...
    8000215e:	82650513          	addi	a0,a0,-2010 # 8000f980 <wait_lock>
    ...
    80002170:	62c98993          	addi	s3,s3,1580 # 80015798 <tickslock>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002174:	0000ec17          	auipc	s8,0xe
    80002178:	80cc0c13          	addi	s8,s8,-2036 # 8000f980 <wait_lock>
    ...
    800021aa:	7da50513          	addi	a0,a0,2010 # 8000f980 <wait_lock>
    ...
    800021d6:	7ae50513          	addi	a0,a0,1966 # 8000f980 <wait_lock>
    ...
    8000221e:	b7e48493          	addi	s1,s1,-1154 # 8000fd98 <proc>
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    800022ee:	c0648493          	addi	s1,s1,-1018 # 8000fef0 <proc+0x158>
    800022f2:	00013917          	auipc	s2,0x13
    800022f6:	5fe90913          	addi	s2,s2,1534 # 800158f0 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
...
trapinit(void)
{
  ...
  initlock(&tickslock, "time");
    ...
    800023ea:	3b250513          	addi	a0,a0,946 # 80015798 <tickslock>
...
trapinithart(void)
{
  ...
  asm volatile("csrw stvec, %0" : : "r" (x));
    ...
    80002404:	07078793          	addi	a5,a5,112 # 80005470 <kernelvec>
...
clockintr()
{
  ...
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    ...
    800024ba:	2e248493          	addi	s1,s1,738 # 80015798 <tickslock>
    800024be:	8526                	mv	a0,s1
    800024c0:	f0efe0ef          	jal	80000bce <acquire>
    ticks++;
    800024c4:	00005517          	auipc	a0,0x5
    800024c8:	3a450513          	addi	a0,a0,932 # 80007868 <ticks>
...
devintr()
{
  ...
  if(scause == 0x8000000000000009L){
    ...
    int irq = plic_claim();
    8000250c:	010030ef          	jal	8000551c <plic_claim>
    ...
    8000252a:	a819                	j	80002540 <devintr+0x60>
      virtio_disk_intr();
    8000252c:	4b6030ef          	jal	800059e2 <virtio_disk_intr>
    if(irq)
    ...
    80002542:	7fb020ef          	jal	8000553c <plic_complete>
    return 1;
...
0000000080002554 <usertrap>:
{
  ...
  asm volatile("csrw stvec, %0" : : "r" (x));
    ...
    8000256e:	f0678793          	addi	a5,a5,-250 # 80005470 <kernelvec>
...
syscall(void)
{
 ...
 // printf("syscall num=%d\n", num); //ADDED FOR DEBUGGING
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    80002862:	37fd                	addiw	a5,a5,-1
    80002864:	4759                	li	a4,22
...
sys_pause(void)
{
  ...
  acquire(&tickslock);
    ...
    800029b4:	de850513          	addi	a0,a0,-536 # 80015798 <tickslock>
    800029b8:	a16fe0ef          	jal	80000bce <acquire>
  ticks0 = ticks;
    800029bc:	00005917          	auipc	s2,0x5
    800029c0:	eac92903          	lw	s2,-340(s2) # 80007868 <ticks>
  while(ticks - ticks0 < n){
    ...
    800029d2:	dca98993          	addi	s3,s3,-566 # 80015798 <tickslock>
    800029d6:	00005497          	auipc	s1,0x5
    800029da:	e9248493          	addi	s1,s1,-366 # 80007868 <ticks>
    if(killed(myproc())){
  ...
  release(&tickslock);
    ...
    80002a06:	d9650513          	addi	a0,a0,-618 # 80015798 <tickslock>
  ...
  return 0;
    ...
    80002a24:	d7850513          	addi	a0,a0,-648 # 80015798 <tickslock>
...
sys_uptime(void)
{
  ...
  release(&tickslock);
    ...
    80002a78:	d2450513          	addi	a0,a0,-732 # 80015798 <tickslock>
...
0000000080002a90 <sys_getcourseno>:
...
sys_getcourseno(void)
{
    ...
    80002a96:	6505                	lui	a0,0x1
    80002a98:	c9550513          	addi	a0,a0,-875 # c95 <_entry-0x7ffff36b>
    80002a9c:	6422                	ld	s0,8(sp)
    80002a9e:	0141                	addi	sp,sp,16
    80002aa0:	8082                	ret

0000000080002aa2 <sys_getprocs>:

uint64
sys_getprocs(void)
{
    80002aa2:	7119                	addi	sp,sp,-128
    80002aa4:	fc86                	sd	ra,120(sp)
    80002aa6:	f8a2                	sd	s0,112(sp)
    80002aa8:	0100                	addi	s0,sp,128
    ...
    80002aaa:	fa840593          	addi	a1,s0,-88
    80002aae:	4501                	li	a0,0
    80002ab0:	d49ff0ef          	jal	800027f8 <argaddr>
				address of process info array */
	argint(1, &max);	/*2nd argument in system call,
    80002ab4:	fa440593          	addi	a1,s0,-92
    80002ab8:	4505                	li	a0,1
    80002aba:	d23ff0ef          	jal	800027dc <argint>
	...
	if(max < 0){
    80002abe:	fa442783          	lw	a5,-92(s0)
	return -1;
    80002ac2:	557d                	li	a0,-1
	if(max < 0){
    80002ac4:	0c07c763          	bltz	a5,80002b92 <sys_getprocs+0xf0>
    80002ac8:	f4a6                	sd	s1,104(sp)
    80002aca:	f0ca                	sd	s2,96(sp)
    80002acc:	ecce                	sd	s3,88(sp)
    80002ace:	e8d2                	sd	s4,80(sp)
    80002ad0:	e4d6                	sd	s5,72(sp)
    80002ad2:	e0da                	sd	s6,64(sp)
    80002ad4:	fc5e                	sd	s7,56(sp)
    ...
    80002adc:	0000d497          	auipc	s1,0xd
    80002ae0:	2bc48493          	addi	s1,s1,700 # 8000fd98 <proc>
	int infoCollected = 0; // number of proccesses information collected 
    80002ae4:	4901                	li	s2,0
	...
	acquire(&wait_lock);
    80002ae6:	0000d997          	auipc	s3,0xd
    80002aea:	e9a98993          	addi	s3,s3,-358 # 8000f980 <wait_lock>
    ...
    80002aee:	4b01                	li	s6,0
	for(p = proc; p < &proc[NPROC] && infoCollected < max; p++){
    80002af0:	00013a97          	auipc	s5,0x13
    80002af4:	ca8a8a93          	addi	s5,s5,-856 # 80015798 <tickslock>
    80002af8:	a085                	j	80002b58 <sys_getprocs+0xb6>
		release(&p->lock);
    80002afa:	8526                	mv	a0,s1
    80002afc:	96afe0ef          	jal	80000c66 <release>
		release(&wait_lock);
    80002b00:	854e                	mv	a0,s3
    80002b02:	964fe0ef          	jal	80000c66 <release>
		continue;
    80002b06:	a0a9                	j	80002b50 <sys_getprocs+0xae>
	pi.ppid = p->parent ? p->parent->pid : 0;
    80002b08:	f8e42223          	sw	a4,-124(s0)
	// does process have parent? access parent pid if true:return 0 if false
	pi.state = p->state;
    80002b0c:	f8f42423          	sw	a5,-120(s0)
	pi.sz = p->sz;
    80002b10:	64bc                	ld	a5,72(s1)
    80002b12:	f8f42623          	sw	a5,-116(s0)
	safestrcpy(pi.name, p->name, sizeof(pi.name));
    80002b16:	4641                	li	a2,16
    80002b18:	158b8593          	addi	a1,s7,344
    80002b1c:	f9040513          	addi	a0,s0,-112
    80002b20:	ac0fe0ef          	jal	80000de0 <safestrcpy>



	release(&p->lock);
    80002b24:	8526                	mv	a0,s1
    80002b26:	940fe0ef          	jal	80000c66 <release>
	release(&wait_lock);
    80002b2a:	854e                	mv	a0,s3
    80002b2c:	93afe0ef          	jal	80000c66 <release>
	//release locks when done accessing process info


	if(copyout(current->pagetable, addr + infoCollected * sizeof(pi),
    80002b30:	00591593          	slli	a1,s2,0x5
    80002b34:	02000693          	li	a3,32
    80002b38:	f8040613          	addi	a2,s0,-128
    80002b3c:	fa843783          	ld	a5,-88(s0)
    80002b40:	95be                	add	a1,a1,a5
    80002b42:	050a3503          	ld	a0,80(s4)
    80002b46:	a9dfe0ef          	jal	800015e2 <copyout>
    80002b4a:	04054863          	bltz	a0,80002b9a <sys_getprocs+0xf8>
		 (char*)&pi, sizeof(pi)) < 0){
    ...
    80002b4e:	2905                	addiw	s2,s2,1
	for(p = proc; p < &proc[NPROC] && infoCollected < max; p++){
    80002b50:	16848493          	addi	s1,s1,360
    80002b54:	03548763          	beq	s1,s5,80002b82 <sys_getprocs+0xe0>
    80002b58:	fa442783          	lw	a5,-92(s0)
    80002b5c:	02f95363          	bge	s2,a5,80002b82 <sys_getprocs+0xe0>
	acquire(&wait_lock);
    80002b60:	854e                	mv	a0,s3
    80002b62:	86cfe0ef          	jal	80000bce <acquire>
	acquire(&p->lock);
    80002b66:	8ba6                	mv	s7,s1
    80002b68:	8526                	mv	a0,s1
    80002b6a:	864fe0ef          	jal	80000bce <acquire>
	if(p->state == UNUSED){
    80002b6e:	4c9c                	lw	a5,24(s1)
    80002b70:	d7c9                	beqz	a5,80002afa <sys_getprocs+0x58>
	pi.pid = p->pid;
    80002b72:	5898                	lw	a4,48(s1)
    80002b74:	f8e42023          	sw	a4,-128(s0)
	pi.ppid = p->parent ? p->parent->pid : 0;
    80002b78:	7c94                	ld	a3,56(s1)
    80002b7a:	875a                	mv	a4,s6
    80002b7c:	d6d1                	beqz	a3,80002b08 <sys_getprocs+0x66>
    80002b7e:	5a98                	lw	a4,48(a3)
    80002b80:	b761                	j	80002b08 <sys_getprocs+0x66>
...
return infoCollected; // return number of information sets collected
    ...
    80002b84:	74a6                	ld	s1,104(sp)
    80002b86:	7906                	ld	s2,96(sp)
    80002b88:	69e6                	ld	s3,88(sp)
    80002b8a:	6a46                	ld	s4,80(sp)
    80002b8c:	6aa6                	ld	s5,72(sp)
    80002b8e:	6b06                	ld	s6,64(sp)
    80002b90:	7be2                	ld	s7,56(sp)

}
    80002b92:	70e6                	ld	ra,120(sp)
    80002b94:	7446                	ld	s0,112(sp)
    80002b96:	6109                	addi	sp,sp,128
    80002b98:	8082                	ret
		return -1; //failure
    80002b9a:	557d                	li	a0,-1
    80002b9c:	74a6                	ld	s1,104(sp)
    80002b9e:	7906                	ld	s2,96(sp)
    80002ba0:	69e6                	ld	s3,88(sp)
    80002ba2:	6a46                	ld	s4,80(sp)
    80002ba4:	6aa6                	ld	s5,72(sp)
    80002ba6:	6b06                	ld	s6,64(sp)
    80002ba8:	7be2                	ld	s7,56(sp)
    80002baa:	b7e5                	j	80002b92 <sys_getprocs+0xf0>
...
binit(void)
{
  ...
  initlock(&bcache.lock, "bcache");
    ...
    80002bc0:	7d458593          	addi	a1,a1,2004 # 80007390 <etext+0x390>
    80002bc4:	00013517          	auipc	a0,0x13
    80002bc8:	bec50513          	addi	a0,a0,-1044 # 800157b0 <bcache>
    80002bcc:	f83fd0ef          	jal	80000b4e <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80002bd0:	0001b797          	auipc	a5,0x1b
    80002bd4:	be078793          	addi	a5,a5,-1056 # 8001d7b0 <bcache+0x8000>
    80002bd8:	0001b717          	auipc	a4,0x1b
    80002bdc:	e4070713          	addi	a4,a4,-448 # 8001da18 <bcache+0x8268>
    80002be0:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002be4:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002be8:	00013497          	auipc	s1,0x13
    80002bec:	be048493          	addi	s1,s1,-1056 # 800157c8 <bcache+0x18>
    b->next = bcache.head.next;
    80002bf0:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80002bf2:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    80002bf4:	00004a17          	auipc	s4,0x4
    80002bf8:	7a4a0a13          	addi	s4,s4,1956 # 80007398 <etext+0x398>
    b->next = bcache.head.next;
    ...
    80002c0c:	322010ef          	jal	80003f2e <initsleeplock>
...
bread(uint dev, uint blockno)
{
  ...
  acquire(&bcache.lock);
    ...
    80002c48:	b6c50513          	addi	a0,a0,-1172 # 800157b0 <bcache>
    80002c4c:	f83fd0ef          	jal	80000bce <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002c50:	0001b497          	auipc	s1,0x1b
    80002c54:	e184b483          	ld	s1,-488(s1) # 8001da68 <bcache+0x82b8>
    80002c58:	0001b797          	auipc	a5,0x1b
    80002c5c:	dc078793          	addi	a5,a5,-576 # 8001da18 <bcache+0x8268>
    ...
    80002c84:	b3050513          	addi	a0,a0,-1232 # 800157b0 <bcache>
    80002c88:	fdffd0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002c8c:	01048513          	addi	a0,s1,16
    80002c90:	2d4010ef          	jal	80003f64 <acquiresleep>
      return b;
    80002c94:	a889                	j	80002ce6 <bread+0xb4>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002c96:	0001b497          	auipc	s1,0x1b
    80002c9a:	dca4b483          	ld	s1,-566(s1) # 8001da60 <bcache+0x82b0>
    80002c9e:	0001b797          	auipc	a5,0x1b
    80002ca2:	d7a78793          	addi	a5,a5,-646 # 8001da18 <bcache+0x8268>
  ...
  panic("bget: no buffers");
    ...
    80002cba:	6ea50513          	addi	a0,a0,1770 # 800073a0 <etext+0x3a0>
    80002cbe:	b23fd0ef          	jal	800007e0 <panic>
      b->dev = dev;
    ...
    80002cd6:	ade50513          	addi	a0,a0,-1314 # 800157b0 <bcache>
    80002cda:	f8dfd0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002cde:	01048513          	addi	a0,s1,16
    80002ce2:	282010ef          	jal	80003f64 <acquiresleep>
  struct buf *b;
  ...
  return b;
    ...
    80002cfe:	2d3020ef          	jal	800057d0 <virtio_disk_rw>
    b->valid = 1;
...
bwrite(struct buf *b)
{
  ...
  if(!holdingsleep(&b->lock))
    ...
    80002d16:	2cc010ef          	jal	80003fe2 <holdingsleep>
    80002d1a:	c911                	beqz	a0,80002d2e <bwrite+0x26>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80002d1c:	4585                	li	a1,1
    80002d1e:	8526                	mv	a0,s1
    80002d20:	2b1020ef          	jal	800057d0 <virtio_disk_rw>
}
    ...
    80002d2c:	8082                	ret
    panic("bwrite");
    80002d2e:	00004517          	auipc	a0,0x4
    80002d32:	68a50513          	addi	a0,a0,1674 # 800073b8 <etext+0x3b8>
    80002d36:	aabfd0ef          	jal	800007e0 <panic>
...
brelse(struct buf *b)
{
  ...
  if(!holdingsleep(&b->lock))
    ...
    80002d4e:	294010ef          	jal	80003fe2 <holdingsleep>
    80002d52:	c135                	beqz	a0,80002db6 <brelse+0x7c>
    panic("brelse");

  releasesleep(&b->lock);
    80002d54:	854a                	mv	a0,s2
    80002d56:	254010ef          	jal	80003faa <releasesleep>

  acquire(&bcache.lock);
    80002d5a:	00013517          	auipc	a0,0x13
    80002d5e:	a5650513          	addi	a0,a0,-1450 # 800157b0 <bcache>
    80002d62:	e6dfd0ef          	jal	80000bce <acquire>
  b->refcnt--;
  ...
  if (b->refcnt == 0) {
    80002d70:	e71d                	bnez	a4,80002d9e <brelse+0x64>
    // no one is waiting for it.
    b->next->prev = b->prev;
    80002d72:	68b8                	ld	a4,80(s1)
    80002d74:	64bc                	ld	a5,72(s1)
    80002d76:	e73c                	sd	a5,72(a4)
    b->prev->next = b->next;
    80002d78:	68b8                	ld	a4,80(s1)
    80002d7a:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    80002d7c:	0001b797          	auipc	a5,0x1b
    80002d80:	a3478793          	addi	a5,a5,-1484 # 8001d7b0 <bcache+0x8000>
    80002d84:	2b87b703          	ld	a4,696(a5)
    80002d88:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002d8a:	0001b717          	auipc	a4,0x1b
    80002d8e:	c8e70713          	addi	a4,a4,-882 # 8001da18 <bcache+0x8268>
    80002d92:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    80002d94:	2b87b703          	ld	a4,696(a5)
    80002d98:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    80002d9a:	2a97bc23          	sd	s1,696(a5)
  }
  ...
  release(&bcache.lock);
    ...
    80002dba:	60a50513          	addi	a0,a0,1546 # 800073c0 <etext+0x3c0>
    80002dbe:	a23fd0ef          	jal	800007e0 <panic>

0000000080002dc2 <bpin>:
...
bpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    ...
    80002dd2:	9e250513          	addi	a0,a0,-1566 # 800157b0 <bcache>
    80002dd6:	df9fd0ef          	jal	80000bce <acquire>
  b->refcnt++;
    80002dda:	40bc                	lw	a5,64(s1)
    80002ddc:	2785                	addiw	a5,a5,1
    80002dde:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002de0:	00013517          	auipc	a0,0x13
    80002de4:	9d050513          	addi	a0,a0,-1584 # 800157b0 <bcache>
    80002de8:	e7ffd0ef          	jal	80000c66 <release>
}
...
bunpin(struct buf *b) {
  ...
  b->refcnt--;
    ...
    80002e10:	37fd                	addiw	a5,a5,-1
    80002e12:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002e14:	00013517          	auipc	a0,0x13
    80002e18:	99c50513          	addi	a0,a0,-1636 # 800157b0 <bcache>
    80002e1c:	e4bfd0ef          	jal	80000c66 <release>
}
...
bfree(int dev, uint b)
{
  ...
  bp = bread(dev, BBLOCK(b, sb));
    ...
    80002e40:	0507a783          	lw	a5,80(a5) # 8001de8c <sb+0x1c>
    80002e44:	9dbd                	addw	a1,a1,a5
    80002e46:	dedff0ef          	jal	80002c32 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    80002e4a:	0074f713          	andi	a4,s1,7
    80002e4e:	4785                	li	a5,1
    80002e50:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    80002e54:	14ce                	slli	s1,s1,0x33
    80002e56:	90d9                	srli	s1,s1,0x36
    80002e58:	00950733          	add	a4,a0,s1
    80002e5c:	05874703          	lbu	a4,88(a4)
    80002e60:	00e7f6b3          	and	a3,a5,a4
    80002e64:	c29d                	beqz	a3,80002e8a <bfree+0x60>
    80002e66:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    80002e68:	94aa                	add	s1,s1,a0
    80002e6a:	fff7c793          	not	a5,a5
    80002e6e:	8f7d                	and	a4,a4,a5
    80002e70:	04e48c23          	sb	a4,88(s1)
  log_write(bp);
    80002e74:	7f9000ef          	jal	80003e6c <log_write>
  brelse(bp);
    80002e78:	854a                	mv	a0,s2
    80002e7a:	ec1ff0ef          	jal	80002d3a <brelse>
}
    ...
    80002e8e:	53e50513          	addi	a0,a0,1342 # 800073c8 <etext+0x3c8>
    80002e92:	94ffd0ef          	jal	800007e0 <panic>

0000000080002e96 <balloc>:
{
    80002e96:	711d                	addi	sp,sp,-96
    80002e98:	ec86                	sd	ra,88(sp)
    80002e9a:	e8a2                	sd	s0,80(sp)
    80002e9c:	e4a6                	sd	s1,72(sp)
    80002e9e:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80002ea0:	0001b797          	auipc	a5,0x1b
    80002ea4:	fd47a783          	lw	a5,-44(a5) # 8001de74 <sb+0x4>
    80002ea8:	0e078f63          	beqz	a5,80002fa6 <balloc+0x110>
    80002eac:	e0ca                	sd	s2,64(sp)
    80002eae:	fc4e                	sd	s3,56(sp)
    80002eb0:	f852                	sd	s4,48(sp)
    80002eb2:	f456                	sd	s5,40(sp)
    80002eb4:	f05a                	sd	s6,32(sp)
    80002eb6:	ec5e                	sd	s7,24(sp)
    80002eb8:	e862                	sd	s8,16(sp)
    80002eba:	e466                	sd	s9,8(sp)
    80002ebc:	8baa                	mv	s7,a0
    80002ebe:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80002ec0:	0001bb17          	auipc	s6,0x1b
    80002ec4:	fb0b0b13          	addi	s6,s6,-80 # 8001de70 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002ec8:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80002eca:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002ecc:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80002ece:	6c89                	lui	s9,0x2
    80002ed0:	a0b5                	j	80002f3c <balloc+0xa6>
        bp->data[bi/8] |= m;  // Mark block in use.
    80002ed2:	97ca                	add	a5,a5,s2
    80002ed4:	8e55                	or	a2,a2,a3
    80002ed6:	04c78c23          	sb	a2,88(a5)
        log_write(bp);
    80002eda:	854a                	mv	a0,s2
    80002edc:	791000ef          	jal	80003e6c <log_write>
        brelse(bp);
    80002ee0:	854a                	mv	a0,s2
    80002ee2:	e59ff0ef          	jal	80002d3a <brelse>
  bp = bread(dev, bno);
    80002ee6:	85a6                	mv	a1,s1
    80002ee8:	855e                	mv	a0,s7
    80002eea:	d49ff0ef          	jal	80002c32 <bread>
    80002eee:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    80002ef0:	40000613          	li	a2,1024
    80002ef4:	4581                	li	a1,0
    80002ef6:	05850513          	addi	a0,a0,88
    80002efa:	da9fd0ef          	jal	80000ca2 <memset>
  log_write(bp);
    80002efe:	854a                	mv	a0,s2
    80002f00:	76d000ef          	jal	80003e6c <log_write>
  brelse(bp);
    80002f04:	854a                	mv	a0,s2
    80002f06:	e35ff0ef          	jal	80002d3a <brelse>
}
    80002f0a:	6906                	ld	s2,64(sp)
    80002f0c:	79e2                	ld	s3,56(sp)
    80002f0e:	7a42                	ld	s4,48(sp)
    80002f10:	7aa2                	ld	s5,40(sp)
    80002f12:	7b02                	ld	s6,32(sp)
    80002f14:	6be2                	ld	s7,24(sp)
    80002f16:	6c42                	ld	s8,16(sp)
    80002f18:	6ca2                	ld	s9,8(sp)
}
    80002f1a:	8526                	mv	a0,s1
    80002f1c:	60e6                	ld	ra,88(sp)
    80002f1e:	6446                	ld	s0,80(sp)
    80002f20:	64a6                	ld	s1,72(sp)
    80002f22:	6125                	addi	sp,sp,96
    80002f24:	8082                	ret
    brelse(bp);
    80002f26:	854a                	mv	a0,s2
    80002f28:	e13ff0ef          	jal	80002d3a <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80002f2c:	015c87bb          	addw	a5,s9,s5
    80002f30:	00078a9b          	sext.w	s5,a5
    80002f34:	004b2703          	lw	a4,4(s6)
    80002f38:	04eaff63          	bgeu	s5,a4,80002f96 <balloc+0x100>
    bp = bread(dev, BBLOCK(b, sb));
    80002f3c:	41fad79b          	sraiw	a5,s5,0x1f
    80002f40:	0137d79b          	srliw	a5,a5,0x13
    80002f44:	015787bb          	addw	a5,a5,s5
    80002f48:	40d7d79b          	sraiw	a5,a5,0xd
    80002f4c:	01cb2583          	lw	a1,28(s6)
    80002f50:	9dbd                	addw	a1,a1,a5
    80002f52:	855e                	mv	a0,s7
    80002f54:	cdfff0ef          	jal	80002c32 <bread>
    80002f58:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002f5a:	004b2503          	lw	a0,4(s6)
    80002f5e:	000a849b          	sext.w	s1,s5
    80002f62:	8762                	mv	a4,s8
    80002f64:	fca4f1e3          	bgeu	s1,a0,80002f26 <balloc+0x90>
      m = 1 << (bi % 8);
    80002f68:	00777693          	andi	a3,a4,7
    80002f6c:	00d996bb          	sllw	a3,s3,a3
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    80002f70:	41f7579b          	sraiw	a5,a4,0x1f
    80002f74:	01d7d79b          	srliw	a5,a5,0x1d
    80002f78:	9fb9                	addw	a5,a5,a4
    80002f7a:	4037d79b          	sraiw	a5,a5,0x3
    80002f7e:	00f90633          	add	a2,s2,a5
    80002f82:	05864603          	lbu	a2,88(a2)
    80002f86:	00c6f5b3          	and	a1,a3,a2
    80002f8a:	d5a1                	beqz	a1,80002ed2 <balloc+0x3c>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002f8c:	2705                	addiw	a4,a4,1
    80002f8e:	2485                	addiw	s1,s1,1
    80002f90:	fd471ae3          	bne	a4,s4,80002f64 <balloc+0xce>
    80002f94:	bf49                	j	80002f26 <balloc+0x90>
    80002f96:	6906                	ld	s2,64(sp)
    80002f98:	79e2                	ld	s3,56(sp)
    80002f9a:	7a42                	ld	s4,48(sp)
    80002f9c:	7aa2                	ld	s5,40(sp)
    80002f9e:	7b02                	ld	s6,32(sp)
    80002fa0:	6be2                	ld	s7,24(sp)
    80002fa2:	6c42                	ld	s8,16(sp)
    80002fa4:	6ca2                	ld	s9,8(sp)
  printf("balloc: out of blocks\n");
    80002fa6:	00004517          	auipc	a0,0x4
    80002faa:	43a50513          	addi	a0,a0,1082 # 800073e0 <etext+0x3e0>
    80002fae:	d4cfd0ef          	jal	800004fa <printf>
  return 0;
    80002fb2:	4481                	li	s1,0
    80002fb4:	b79d                	j	80002f1a <balloc+0x84>

0000000080002fb6 <bmap>:
...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    80002fc6:	47ad                	li	a5,11
    80002fc8:	02b7e663          	bltu	a5,a1,80002ff4 <bmap+0x3e>
    if((addr = ip->addrs[bn]) == 0){
    80002fcc:	02059793          	slli	a5,a1,0x20
    80002fd0:	01e7d593          	srli	a1,a5,0x1e
    80002fd4:	00b504b3          	add	s1,a0,a1
    80002fd8:	0504a903          	lw	s2,80(s1)
    80002fdc:	06091a63          	bnez	s2,80003050 <bmap+0x9a>
      addr = balloc(ip->dev);
    80002fe0:	4108                	lw	a0,0(a0)
    80002fe2:	eb5ff0ef          	jal	80002e96 <balloc>
    80002fe6:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002fea:	06090363          	beqz	s2,80003050 <bmap+0x9a>
        return 0;
      ip->addrs[bn] = addr;
    80002fee:	0524a823          	sw	s2,80(s1)
    80002ff2:	a8b9                	j	80003050 <bmap+0x9a>
    }
    return addr;
  }
  bn -= NDIRECT;
    80002ff4:	ff45849b          	addiw	s1,a1,-12
    80002ff8:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80002ffc:	0ff00793          	li	a5,255
    80003000:	06e7ee63          	bltu	a5,a4,8000307c <bmap+0xc6>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0){
    80003004:	08052903          	lw	s2,128(a0)
    80003008:	00091d63          	bnez	s2,80003022 <bmap+0x6c>
      addr = balloc(ip->dev);
    8000300c:	4108                	lw	a0,0(a0)
    8000300e:	e89ff0ef          	jal	80002e96 <balloc>
    80003012:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80003016:	02090d63          	beqz	s2,80003050 <bmap+0x9a>
    8000301a:	e052                	sd	s4,0(sp)
        return 0;
      ip->addrs[NDIRECT] = addr;
    8000301c:	0929a023          	sw	s2,128(s3)
    80003020:	a011                	j	80003024 <bmap+0x6e>
    80003022:	e052                	sd	s4,0(sp)
    }
    bp = bread(ip->dev, addr);
    80003024:	85ca                	mv	a1,s2
    80003026:	0009a503          	lw	a0,0(s3)
    8000302a:	c09ff0ef          	jal	80002c32 <bread>
    8000302e:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    80003030:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    80003034:	02049713          	slli	a4,s1,0x20
    80003038:	01e75593          	srli	a1,a4,0x1e
    8000303c:	00b784b3          	add	s1,a5,a1
    80003040:	0004a903          	lw	s2,0(s1)
    80003044:	00090e63          	beqz	s2,80003060 <bmap+0xaa>
      if(addr){
        a[bn] = addr;
        log_write(bp);
      }
    ...
    8000304a:	cf1ff0ef          	jal	80002d3a <brelse>
    return addr;
  ...
  panic("bmap: out of range");
    ...
    80003064:	e33ff0ef          	jal	80002e96 <balloc>
    80003068:	0005091b          	sext.w	s2,a0
      if(addr){
    8000306c:	fc090ee3          	beqz	s2,80003048 <bmap+0x92>
        a[bn] = addr;
    80003070:	0124a023          	sw	s2,0(s1)
        log_write(bp);
    80003074:	8552                	mv	a0,s4
    80003076:	5f7000ef          	jal	80003e6c <log_write>
    8000307a:	b7f9                	j	80003048 <bmap+0x92>
    8000307c:	e052                	sd	s4,0(sp)
  panic("bmap: out of range");
    8000307e:	00004517          	auipc	a0,0x4
    80003082:	37a50513          	addi	a0,a0,890 # 800073f8 <etext+0x3f8>
    80003086:	f5afd0ef          	jal	800007e0 <panic>

000000008000308a <iget>:
{
    ...
    8000309c:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    8000309e:	0001b517          	auipc	a0,0x1b
    800030a2:	df250513          	addi	a0,a0,-526 # 8001de90 <itable>
    800030a6:	b29fd0ef          	jal	80000bce <acquire>
  empty = 0;
    800030aa:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    800030ac:	0001b497          	auipc	s1,0x1b
    800030b0:	dfc48493          	addi	s1,s1,-516 # 8001dea8 <itable+0x18>
    800030b4:	0001d697          	auipc	a3,0x1d
    800030b8:	88468693          	addi	a3,a3,-1916 # 8001f938 <log>
    800030bc:	a039                	j	800030ca <iget+0x40>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    800030be:	02090963          	beqz	s2,800030f0 <iget+0x66>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    800030c2:	08848493          	addi	s1,s1,136
    800030c6:	02d48863          	beq	s1,a3,800030f6 <iget+0x6c>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    800030ca:	449c                	lw	a5,8(s1)
    800030cc:	fef059e3          	blez	a5,800030be <iget+0x34>
    800030d0:	4098                	lw	a4,0(s1)
    800030d2:	ff3716e3          	bne	a4,s3,800030be <iget+0x34>
    800030d6:	40d8                	lw	a4,4(s1)
    800030d8:	ff4713e3          	bne	a4,s4,800030be <iget+0x34>
      ip->ref++;
    800030dc:	2785                	addiw	a5,a5,1
    800030de:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    800030e0:	0001b517          	auipc	a0,0x1b
    800030e4:	db050513          	addi	a0,a0,-592 # 8001de90 <itable>
    800030e8:	b7ffd0ef          	jal	80000c66 <release>
      return ip;
    800030ec:	8926                	mv	s2,s1
    800030ee:	a02d                	j	80003118 <iget+0x8e>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    800030f0:	fbe9                	bnez	a5,800030c2 <iget+0x38>
      empty = ip;
    800030f2:	8926                	mv	s2,s1
    800030f4:	b7f9                	j	800030c2 <iget+0x38>
  if(empty == 0)
    800030f6:	02090a63          	beqz	s2,8000312a <iget+0xa0>
  ip->dev = dev;
    800030fa:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    800030fe:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    80003102:	4785                	li	a5,1
    80003104:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    80003108:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    8000310c:	0001b517          	auipc	a0,0x1b
    80003110:	d8450513          	addi	a0,a0,-636 # 8001de90 <itable>
    80003114:	b53fd0ef          	jal	80000c66 <release>
}
...
0000000080003136 <iinit>:
{
  ...
  initlock(&itable.lock, "itable");
    ...
    80003148:	2dc58593          	addi	a1,a1,732 # 80007420 <etext+0x420>
    8000314c:	0001b517          	auipc	a0,0x1b
    80003150:	d4450513          	addi	a0,a0,-700 # 8001de90 <itable>
    80003154:	9fbfd0ef          	jal	80000b4e <initlock>
  for(i = 0; i < NINODE; i++) {
    80003158:	0001b497          	auipc	s1,0x1b
    8000315c:	d6048493          	addi	s1,s1,-672 # 8001deb8 <itable+0x28>
    80003160:	0001c997          	auipc	s3,0x1c
    80003164:	7e898993          	addi	s3,s3,2024 # 8001f948 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    80003168:	00004917          	auipc	s2,0x4
    8000316c:	2c090913          	addi	s2,s2,704 # 80007428 <etext+0x428>
    80003170:	85ca                	mv	a1,s2
    80003172:	8526                	mv	a0,s1
    80003174:	5bb000ef          	jal	80003f2e <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    80003178:	08848493          	addi	s1,s1,136
    8000317c:	ff349ae3          	bne	s1,s3,80003170 <iinit+0x3a>
}
...
000000008000318e <ialloc>:
{
    8000318e:	7139                	addi	sp,sp,-64
    80003190:	fc06                	sd	ra,56(sp)
    80003192:	f822                	sd	s0,48(sp)
    80003194:	0080                	addi	s0,sp,64
  for(inum = 1; inum < sb.ninodes; inum++){
    80003196:	0001b717          	auipc	a4,0x1b
    8000319a:	ce672703          	lw	a4,-794(a4) # 8001de7c <sb+0xc>
    8000319e:	4785                	li	a5,1
    800031a0:	06e7f063          	bgeu	a5,a4,80003200 <ialloc+0x72>
    800031a4:	f426                	sd	s1,40(sp)
    800031a6:	f04a                	sd	s2,32(sp)
    800031a8:	ec4e                	sd	s3,24(sp)
    800031aa:	e852                	sd	s4,16(sp)
    800031ac:	e456                	sd	s5,8(sp)
    800031ae:	e05a                	sd	s6,0(sp)
    800031b0:	8aaa                	mv	s5,a0
    800031b2:	8b2e                	mv	s6,a1
    800031b4:	4905                	li	s2,1
    bp = bread(dev, IBLOCK(inum, sb));
    800031b6:	0001ba17          	auipc	s4,0x1b
    800031ba:	cbaa0a13          	addi	s4,s4,-838 # 8001de70 <sb>
    800031be:	00495593          	srli	a1,s2,0x4
    800031c2:	018a2783          	lw	a5,24(s4)
    800031c6:	9dbd                	addw	a1,a1,a5
    800031c8:	8556                	mv	a0,s5
    800031ca:	a69ff0ef          	jal	80002c32 <bread>
    800031ce:	84aa                	mv	s1,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    800031d0:	05850993          	addi	s3,a0,88
    800031d4:	00f97793          	andi	a5,s2,15
    800031d8:	079a                	slli	a5,a5,0x6
    800031da:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    800031dc:	00099783          	lh	a5,0(s3)
    800031e0:	cb9d                	beqz	a5,80003216 <ialloc+0x88>
    brelse(bp);
    800031e2:	b59ff0ef          	jal	80002d3a <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    800031e6:	0905                	addi	s2,s2,1
    800031e8:	00ca2703          	lw	a4,12(s4)
    800031ec:	0009079b          	sext.w	a5,s2
    800031f0:	fce7e7e3          	bltu	a5,a4,800031be <ialloc+0x30>
    800031f4:	74a2                	ld	s1,40(sp)
    800031f6:	7902                	ld	s2,32(sp)
    800031f8:	69e2                	ld	s3,24(sp)
    800031fa:	6a42                	ld	s4,16(sp)
    800031fc:	6aa2                	ld	s5,8(sp)
    800031fe:	6b02                	ld	s6,0(sp)
  printf("ialloc: no inodes\n");
    80003200:	00004517          	auipc	a0,0x4
    80003204:	23050513          	addi	a0,a0,560 # 80007430 <etext+0x430>
    80003208:	af2fd0ef          	jal	800004fa <printf>
  return 0;
    ...
    80003216:	04000613          	li	a2,64
    8000321a:	4581                	li	a1,0
    8000321c:	854e                	mv	a0,s3
    8000321e:	a85fd0ef          	jal	80000ca2 <memset>
      dip->type = type;
    80003222:	01699023          	sh	s6,0(s3)
      log_write(bp);   // mark it allocated on the disk
    80003226:	8526                	mv	a0,s1
    80003228:	445000ef          	jal	80003e6c <log_write>
      brelse(bp);
    8000322c:	8526                	mv	a0,s1
    8000322e:	b0dff0ef          	jal	80002d3a <brelse>
      return iget(dev, inum);
    80003232:	0009059b          	sext.w	a1,s2
    80003236:	8556                	mv	a0,s5
    80003238:	e53ff0ef          	jal	8000308a <iget>
    ...
    80003246:	6b02                	ld	s6,0(sp)
    80003248:	b7d9                	j	8000320e <ialloc+0x80>

000000008000324a <iupdate>:
{
  ...
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003258:	415c                	lw	a5,4(a0)
    8000325a:	0047d79b          	srliw	a5,a5,0x4
    8000325e:	0001b597          	auipc	a1,0x1b
    80003262:	c2a5a583          	lw	a1,-982(a1) # 8001de88 <sb+0x18>
    80003266:	9dbd                	addw	a1,a1,a5
    80003268:	4108                	lw	a0,0(a0)
    8000326a:	9c9ff0ef          	jal	80002c32 <bread>
    8000326e:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003270:	05850793          	addi	a5,a0,88
    80003274:	40d8                	lw	a4,4(s1)
    80003276:	8b3d                	andi	a4,a4,15
    80003278:	071a                	slli	a4,a4,0x6
    8000327a:	97ba                	add	a5,a5,a4
  dip->type = ip->type;
    8000327c:	04449703          	lh	a4,68(s1)
    80003280:	00e79023          	sh	a4,0(a5)
  dip->major = ip->major;
    80003284:	04649703          	lh	a4,70(s1)
    80003288:	00e79123          	sh	a4,2(a5)
  dip->minor = ip->minor;
    8000328c:	04849703          	lh	a4,72(s1)
    80003290:	00e79223          	sh	a4,4(a5)
  dip->nlink = ip->nlink;
    80003294:	04a49703          	lh	a4,74(s1)
    80003298:	00e79323          	sh	a4,6(a5)
  dip->size = ip->size;
    8000329c:	44f8                	lw	a4,76(s1)
    8000329e:	c798                	sw	a4,8(a5)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    800032a0:	03400613          	li	a2,52
    800032a4:	05048593          	addi	a1,s1,80
    800032a8:	00c78513          	addi	a0,a5,12
    800032ac:	a53fd0ef          	jal	80000cfe <memmove>
  log_write(bp);
    800032b0:	854a                	mv	a0,s2
    800032b2:	3bb000ef          	jal	80003e6c <log_write>
  brelse(bp);
    800032b6:	854a                	mv	a0,s2
    800032b8:	a83ff0ef          	jal	80002d3a <brelse>
}
...
00000000800032c8 <idup>:
{
  ...
  acquire(&itable.lock);
    800032d4:	0001b517          	auipc	a0,0x1b
    800032d8:	bbc50513          	addi	a0,a0,-1092 # 8001de90 <itable>
    800032dc:	8f3fd0ef          	jal	80000bce <acquire>
  ip->ref++;
    800032e0:	449c                	lw	a5,8(s1)
    800032e2:	2785                	addiw	a5,a5,1
    800032e4:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    800032e6:	0001b517          	auipc	a0,0x1b
    800032ea:	baa50513          	addi	a0,a0,-1110 # 8001de90 <itable>
    800032ee:	979fd0ef          	jal	80000c66 <release>
}
...
00000000800032fe <ilock>:
{
  ...
  if(ip == 0 || ip->ref < 1)
    80003308:	cd19                	beqz	a0,80003326 <ilock+0x28>
    8000330a:	84aa                	mv	s1,a0
    8000330c:	451c                	lw	a5,8(a0)
    8000330e:	00f05c63          	blez	a5,80003326 <ilock+0x28>
  acquiresleep(&ip->lock);
    80003312:	0541                	addi	a0,a0,16
    80003314:	451000ef          	jal	80003f64 <acquiresleep>
  if(ip->valid == 0){
    80003318:	40bc                	lw	a5,64(s1)
    8000331a:	cf89                	beqz	a5,80003334 <ilock+0x36>
}
    ...
    8000332c:	12050513          	addi	a0,a0,288 # 80007448 <etext+0x448>
    80003330:	cb0fd0ef          	jal	800007e0 <panic>
    80003334:	e04a                	sd	s2,0(sp)
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003336:	40dc                	lw	a5,4(s1)
    80003338:	0047d79b          	srliw	a5,a5,0x4
    8000333c:	0001b597          	auipc	a1,0x1b
    80003340:	b4c5a583          	lw	a1,-1204(a1) # 8001de88 <sb+0x18>
    80003344:	9dbd                	addw	a1,a1,a5
    80003346:	4088                	lw	a0,0(s1)
    80003348:	8ebff0ef          	jal	80002c32 <bread>
    8000334c:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    8000334e:	05850593          	addi	a1,a0,88
    80003352:	40dc                	lw	a5,4(s1)
    80003354:	8bbd                	andi	a5,a5,15
    80003356:	079a                	slli	a5,a5,0x6
    80003358:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    8000335a:	00059783          	lh	a5,0(a1)
    8000335e:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    80003362:	00259783          	lh	a5,2(a1)
    80003366:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    8000336a:	00459783          	lh	a5,4(a1)
    8000336e:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    80003372:	00659783          	lh	a5,6(a1)
    80003376:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    8000337a:	459c                	lw	a5,8(a1)
    8000337c:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    8000337e:	03400613          	li	a2,52
    80003382:	05b1                	addi	a1,a1,12
    80003384:	05048513          	addi	a0,s1,80
    80003388:	977fd0ef          	jal	80000cfe <memmove>
    brelse(bp);
    8000338c:	854a                	mv	a0,s2
    8000338e:	9adff0ef          	jal	80002d3a <brelse>
    ip->valid = 1;
    80003392:	4785                	li	a5,1
    80003394:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    80003396:	04449783          	lh	a5,68(s1)
    8000339a:	c399                	beqz	a5,800033a0 <ilock+0xa2>
    8000339c:	6902                	ld	s2,0(sp)
    8000339e:	bfbd                	j	8000331c <ilock+0x1e>
      panic("ilock: no type");
    800033a0:	00004517          	auipc	a0,0x4
    800033a4:	0b050513          	addi	a0,a0,176 # 80007450 <etext+0x450>
    800033a8:	c38fd0ef          	jal	800007e0 <panic>

00000000800033ac <iunlock>:
{
  ...
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    800033b8:	c505                	beqz	a0,800033e0 <iunlock+0x34>
    800033ba:	84aa                	mv	s1,a0
    800033bc:	01050913          	addi	s2,a0,16
    800033c0:	854a                	mv	a0,s2
    800033c2:	421000ef          	jal	80003fe2 <holdingsleep>
    800033c6:	cd09                	beqz	a0,800033e0 <iunlock+0x34>
    800033c8:	449c                	lw	a5,8(s1)
    800033ca:	00f05b63          	blez	a5,800033e0 <iunlock+0x34>
  releasesleep(&ip->lock);
    800033ce:	854a                	mv	a0,s2
    800033d0:	3db000ef          	jal	80003faa <releasesleep>
}
    ...
    800033e4:	08050513          	addi	a0,a0,128 # 80007460 <etext+0x460>
    800033e8:	bf8fd0ef          	jal	800007e0 <panic>

00000000800033ec <itrunc>:
...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    800033fc:	05050493          	addi	s1,a0,80
    80003400:	08050913          	addi	s2,a0,128
    80003404:	a021                	j	8000340c <itrunc+0x20>
    80003406:	0491                	addi	s1,s1,4
    80003408:	01248b63          	beq	s1,s2,8000341e <itrunc+0x32>
    if(ip->addrs[i]){
    8000340c:	408c                	lw	a1,0(s1)
    8000340e:	dde5                	beqz	a1,80003406 <itrunc+0x1a>
      bfree(ip->dev, ip->addrs[i]);
    80003410:	0009a503          	lw	a0,0(s3)
    80003414:	a17ff0ef          	jal	80002e2a <bfree>
      ip->addrs[i] = 0;
    80003418:	0004a023          	sw	zero,0(s1)
    8000341c:	b7ed                	j	80003406 <itrunc+0x1a>
    }
  }

  if(ip->addrs[NDIRECT]){
    8000341e:	0809a583          	lw	a1,128(s3)
    80003422:	ed89                	bnez	a1,8000343c <itrunc+0x50>
    brelse(bp);
  ...
  ip->size = 0;
    80003424:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    80003428:	854e                	mv	a0,s3
    8000342a:	e21ff0ef          	jal	8000324a <iupdate>
}
    ...
    80003442:	ff0ff0ef          	jal	80002c32 <bread>
    80003446:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    80003448:	05850493          	addi	s1,a0,88
    8000344c:	45850913          	addi	s2,a0,1112
    80003450:	a021                	j	80003458 <itrunc+0x6c>
    80003452:	0491                	addi	s1,s1,4
    80003454:	01248963          	beq	s1,s2,80003466 <itrunc+0x7a>
      if(a[j])
    80003458:	408c                	lw	a1,0(s1)
    8000345a:	dde5                	beqz	a1,80003452 <itrunc+0x66>
        bfree(ip->dev, a[j]);
    8000345c:	0009a503          	lw	a0,0(s3)
    80003460:	9cbff0ef          	jal	80002e2a <bfree>
    80003464:	b7fd                	j	80003452 <itrunc+0x66>
    brelse(bp);
    80003466:	8552                	mv	a0,s4
    80003468:	8d3ff0ef          	jal	80002d3a <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    8000346c:	0809a583          	lw	a1,128(s3)
    80003470:	0009a503          	lw	a0,0(s3)
    80003474:	9b7ff0ef          	jal	80002e2a <bfree>
    ip->addrs[NDIRECT] = 0;
    80003478:	0809a023          	sw	zero,128(s3)
    8000347c:	6a02                	ld	s4,0(sp)
    8000347e:	b75d                	j	80003424 <itrunc+0x38>

0000000080003480 <iput>:
{
  ...
  acquire(&itable.lock);
    ...
    80003490:	a0450513          	addi	a0,a0,-1532 # 8001de90 <itable>
    80003494:	f3afd0ef          	jal	80000bce <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003498:	4498                	lw	a4,8(s1)
    8000349a:	4785                	li	a5,1
    8000349c:	02f70063          	beq	a4,a5,800034bc <iput+0x3c>
  ip->ref--;
    800034a0:	449c                	lw	a5,8(s1)
    800034a2:	37fd                	addiw	a5,a5,-1
    800034a4:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    800034a6:	0001b517          	auipc	a0,0x1b
    800034aa:	9ea50513          	addi	a0,a0,-1558 # 8001de90 <itable>
    800034ae:	fb8fd0ef          	jal	80000c66 <release>
}
  ...
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    ...
    800034be:	d3ed                	beqz	a5,800034a0 <iput+0x20>
    800034c0:	04a49783          	lh	a5,74(s1)
    800034c4:	fff1                	bnez	a5,800034a0 <iput+0x20>
    800034c6:	e04a                	sd	s2,0(sp)
    acquiresleep(&ip->lock);
    800034c8:	01048913          	addi	s2,s1,16
    800034cc:	854a                	mv	a0,s2
    800034ce:	297000ef          	jal	80003f64 <acquiresleep>
    release(&itable.lock);
    800034d2:	0001b517          	auipc	a0,0x1b
    800034d6:	9be50513          	addi	a0,a0,-1602 # 8001de90 <itable>
    800034da:	f8cfd0ef          	jal	80000c66 <release>
    itrunc(ip);
    800034de:	8526                	mv	a0,s1
    800034e0:	f0dff0ef          	jal	800033ec <itrunc>
    ip->type = 0;
    800034e4:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    800034e8:	8526                	mv	a0,s1
    800034ea:	d61ff0ef          	jal	8000324a <iupdate>
    ip->valid = 0;
    800034ee:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    800034f2:	854a                	mv	a0,s2
    800034f4:	2b7000ef          	jal	80003faa <releasesleep>
    acquire(&itable.lock);
    800034f8:	0001b517          	auipc	a0,0x1b
    800034fc:	99850513          	addi	a0,a0,-1640 # 8001de90 <itable>
    80003500:	ecefd0ef          	jal	80000bce <acquire>
    80003504:	6902                	ld	s2,0(sp)
    80003506:	bf69                	j	800034a0 <iput+0x20>

0000000080003508 <iunlockput>:
{
  ...
  iunlock(ip);
    80003514:	e99ff0ef          	jal	800033ac <iunlock>
  iput(ip);
    80003518:	8526                	mv	a0,s1
    8000351a:	f67ff0ef          	jal	80003480 <iput>
}
...
0000000080003528 <ireclaim>:
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003528:	0001b717          	auipc	a4,0x1b
    8000352c:	95472703          	lw	a4,-1708(a4) # 8001de7c <sb+0xc>
    80003530:	4785                	li	a5,1
    80003532:	0ae7ff63          	bgeu	a5,a4,800035f0 <ireclaim+0xc8>
{
    ...
    80003546:	e05a                	sd	s6,0(sp)
    80003548:	0080                	addi	s0,sp,64
  for (int inum = 1; inum < sb.ninodes; inum++) {
    8000354a:	4485                	li	s1,1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    8000354c:	00050a1b          	sext.w	s4,a0
    80003550:	0001ba97          	auipc	s5,0x1b
    80003554:	920a8a93          	addi	s5,s5,-1760 # 8001de70 <sb>
      printf("ireclaim: orphaned inode %d\n", inum);
    80003558:	00004b17          	auipc	s6,0x4
    8000355c:	f10b0b13          	addi	s6,s6,-240 # 80007468 <etext+0x468>
    80003560:	a099                	j	800035a6 <ireclaim+0x7e>
    80003562:	85ce                	mv	a1,s3
    80003564:	855a                	mv	a0,s6
    80003566:	f95fc0ef          	jal	800004fa <printf>
      ip = iget(dev, inum);
    8000356a:	85ce                	mv	a1,s3
    8000356c:	8552                	mv	a0,s4
    8000356e:	b1dff0ef          	jal	8000308a <iget>
    80003572:	89aa                	mv	s3,a0
    brelse(bp);
    80003574:	854a                	mv	a0,s2
    80003576:	fc4ff0ef          	jal	80002d3a <brelse>
    if (ip) {
    8000357a:	00098f63          	beqz	s3,80003598 <ireclaim+0x70>
      begin_op();
    8000357e:	76a000ef          	jal	80003ce8 <begin_op>
      ilock(ip);
    80003582:	854e                	mv	a0,s3
    80003584:	d7bff0ef          	jal	800032fe <ilock>
      iunlock(ip);
    80003588:	854e                	mv	a0,s3
    8000358a:	e23ff0ef          	jal	800033ac <iunlock>
      iput(ip);
    8000358e:	854e                	mv	a0,s3
    80003590:	ef1ff0ef          	jal	80003480 <iput>
      end_op();
    80003594:	7be000ef          	jal	80003d52 <end_op>
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003598:	0485                	addi	s1,s1,1
    8000359a:	00caa703          	lw	a4,12(s5)
    8000359e:	0004879b          	sext.w	a5,s1
    800035a2:	02e7fd63          	bgeu	a5,a4,800035dc <ireclaim+0xb4>
    800035a6:	0004899b          	sext.w	s3,s1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    800035aa:	0044d593          	srli	a1,s1,0x4
    800035ae:	018aa783          	lw	a5,24(s5)
    800035b2:	9dbd                	addw	a1,a1,a5
    800035b4:	8552                	mv	a0,s4
    800035b6:	e7cff0ef          	jal	80002c32 <bread>
    800035ba:	892a                	mv	s2,a0
    struct dinode *dip = (struct dinode *)bp->data + inum % IPB;
    800035bc:	05850793          	addi	a5,a0,88
    800035c0:	00f9f713          	andi	a4,s3,15
    800035c4:	071a                	slli	a4,a4,0x6
    800035c6:	97ba                	add	a5,a5,a4
    if (dip->type != 0 && dip->nlink == 0) {  // is an orphaned inode
    800035c8:	00079703          	lh	a4,0(a5)
    800035cc:	c701                	beqz	a4,800035d4 <ireclaim+0xac>
    800035ce:	00679783          	lh	a5,6(a5)
    800035d2:	dbc1                	beqz	a5,80003562 <ireclaim+0x3a>
    brelse(bp);
    800035d4:	854a                	mv	a0,s2
    800035d6:	f64ff0ef          	jal	80002d3a <brelse>
    if (ip) {
    800035da:	bf7d                	j	80003598 <ireclaim+0x70>
}
...
fsinit(int dev) {
  ...
  bp = bread(dev, 1);
    ...
    80003604:	e2eff0ef          	jal	80002c32 <bread>
    80003608:	892a                	mv	s2,a0
  memmove(sb, bp->data, sizeof(*sb));
    8000360a:	0001b997          	auipc	s3,0x1b
    8000360e:	86698993          	addi	s3,s3,-1946 # 8001de70 <sb>
    80003612:	02000613          	li	a2,32
    80003616:	05850593          	addi	a1,a0,88
    8000361a:	854e                	mv	a0,s3
    8000361c:	ee2fd0ef          	jal	80000cfe <memmove>
  brelse(bp);
    80003620:	854a                	mv	a0,s2
    80003622:	f18ff0ef          	jal	80002d3a <brelse>
  if(sb.magic != FSMAGIC)
    80003626:	0009a703          	lw	a4,0(s3)
    8000362a:	102037b7          	lui	a5,0x10203
    8000362e:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    80003632:	02f71363          	bne	a4,a5,80003658 <fsinit+0x66>
  initlog(dev, &sb);
    80003636:	0001b597          	auipc	a1,0x1b
    8000363a:	83a58593          	addi	a1,a1,-1990 # 8001de70 <sb>
    8000363e:	8526                	mv	a0,s1
    80003640:	62a000ef          	jal	80003c6a <initlog>
  ireclaim(dev);
    80003644:	8526                	mv	a0,s1
    80003646:	ee3ff0ef          	jal	80003528 <ireclaim>
}
    ...
    80003658:	00004517          	auipc	a0,0x4
    8000365c:	e3050513          	addi	a0,a0,-464 # 80007488 <etext+0x488>
    80003660:	980fd0ef          	jal	800007e0 <panic>

0000000080003664 <stati>:
...
stati(struct inode *ip, struct stat *st)
{
    ...
    80003666:	e422                	sd	s0,8(sp)
    80003668:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    8000366a:	411c                	lw	a5,0(a0)
    8000366c:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    8000366e:	415c                	lw	a5,4(a0)
    80003670:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    80003672:	04451783          	lh	a5,68(a0)
    80003676:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    8000367a:	04a51783          	lh	a5,74(a0)
    8000367e:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    80003682:	04c56783          	lwu	a5,76(a0)
    80003686:	e99c                	sd	a5,16(a1)
}
    80003688:	6422                	ld	s0,8(sp)
    8000368a:	0141                	addi	sp,sp,16
    8000368c:	8082                	ret

000000008000368e <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  ...
  if(off > ip->size || off + n < off)
    8000368e:	457c                	lw	a5,76(a0)
    80003690:	0ed7eb63          	bltu	a5,a3,80003786 <readi+0xf8>
{
    80003694:	7159                	addi	sp,sp,-112
    80003696:	f486                	sd	ra,104(sp)
    80003698:	f0a2                	sd	s0,96(sp)
    8000369a:	eca6                	sd	s1,88(sp)
    8000369c:	e0d2                	sd	s4,64(sp)
    8000369e:	fc56                	sd	s5,56(sp)
    800036a0:	f85a                	sd	s6,48(sp)
    800036a2:	f45e                	sd	s7,40(sp)
    800036a4:	1880                	addi	s0,sp,112
    800036a6:	8b2a                	mv	s6,a0
    800036a8:	8bae                	mv	s7,a1
    800036aa:	8a32                	mv	s4,a2
    800036ac:	84b6                	mv	s1,a3
    800036ae:	8aba                	mv	s5,a4
  if(off > ip->size || off + n < off)
    800036b0:	9f35                	addw	a4,a4,a3
    return 0;
    800036b2:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    800036b4:	0cd76063          	bltu	a4,a3,80003774 <readi+0xe6>
    800036b8:	e4ce                	sd	s3,72(sp)
  if(off + n > ip->size)
    800036ba:	00e7f463          	bgeu	a5,a4,800036c2 <readi+0x34>
    n = ip->size - off;
    800036be:	40d78abb          	subw	s5,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    800036c2:	080a8f63          	beqz	s5,80003760 <readi+0xd2>
    800036c6:	e8ca                	sd	s2,80(sp)
    800036c8:	f062                	sd	s8,32(sp)
    800036ca:	ec66                	sd	s9,24(sp)
    800036cc:	e86a                	sd	s10,16(sp)
    800036ce:	e46e                	sd	s11,8(sp)
    800036d0:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    800036d2:	40000c93          	li	s9,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    800036d6:	5c7d                	li	s8,-1
    800036d8:	a80d                	j	8000370a <readi+0x7c>
    800036da:	020d1d93          	slli	s11,s10,0x20
    800036de:	020ddd93          	srli	s11,s11,0x20
    800036e2:	05890613          	addi	a2,s2,88
    800036e6:	86ee                	mv	a3,s11
    800036e8:	963a                	add	a2,a2,a4
    800036ea:	85d2                	mv	a1,s4
    800036ec:	855e                	mv	a0,s7
    800036ee:	b47fe0ef          	jal	80002234 <either_copyout>
    800036f2:	05850763          	beq	a0,s8,80003740 <readi+0xb2>
      brelse(bp);
      tot = -1;
      break;
    }
    ...
    800036f8:	e42ff0ef          	jal	80002d3a <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    800036fc:	013d09bb          	addw	s3,s10,s3
    80003700:	009d04bb          	addw	s1,s10,s1
    80003704:	9a6e                	add	s4,s4,s11
    80003706:	0559f763          	bgeu	s3,s5,80003754 <readi+0xc6>
    uint addr = bmap(ip, off/BSIZE);
    8000370a:	00a4d59b          	srliw	a1,s1,0xa
    8000370e:	855a                	mv	a0,s6
    80003710:	8a7ff0ef          	jal	80002fb6 <bmap>
    80003714:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    80003718:	c5b1                	beqz	a1,80003764 <readi+0xd6>
    bp = bread(ip->dev, addr);
    8000371a:	000b2503          	lw	a0,0(s6)
    8000371e:	d14ff0ef          	jal	80002c32 <bread>
    80003722:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003724:	3ff4f713          	andi	a4,s1,1023
    80003728:	40ec87bb          	subw	a5,s9,a4
    8000372c:	413a86bb          	subw	a3,s5,s3
    80003730:	8d3e                	mv	s10,a5
    80003732:	2781                	sext.w	a5,a5
    80003734:	0006861b          	sext.w	a2,a3
    80003738:	faf671e3          	bgeu	a2,a5,800036da <readi+0x4c>
    8000373c:	8d36                	mv	s10,a3
    8000373e:	bf71                	j	800036da <readi+0x4c>
      brelse(bp);
    80003740:	854a                	mv	a0,s2
    80003742:	df8ff0ef          	jal	80002d3a <brelse>
      tot = -1;
    80003746:	59fd                	li	s3,-1
      break;
    ...
    80003752:	a831                	j	8000376e <readi+0xe0>
    ...
    8000375e:	a801                	j	8000376e <readi+0xe0>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003760:	89d6                	mv	s3,s5
    80003762:	a031                	j	8000376e <readi+0xe0>
...
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  ...
  if(off > ip->size || off + n < off)
    ...
    8000378c:	10d7e063          	bltu	a5,a3,8000388c <writei+0x102>
{
    80003790:	7159                	addi	sp,sp,-112
    80003792:	f486                	sd	ra,104(sp)
    80003794:	f0a2                	sd	s0,96(sp)
    80003796:	e8ca                	sd	s2,80(sp)
    80003798:	e0d2                	sd	s4,64(sp)
    8000379a:	fc56                	sd	s5,56(sp)
  ...
  if(off > ip->size || off + n < off)
    ...
    800037b0:	0ed7e063          	bltu	a5,a3,80003890 <writei+0x106>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    800037b4:	00043737          	lui	a4,0x43
    800037b8:	0cf76e63          	bltu	a4,a5,80003894 <writei+0x10a>
    800037bc:	e4ce                	sd	s3,72(sp)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    800037be:	0a0b0f63          	beqz	s6,8000387c <writei+0xf2>
    800037c2:	eca6                	sd	s1,88(sp)
    800037c4:	f062                	sd	s8,32(sp)
    800037c6:	ec66                	sd	s9,24(sp)
    800037c8:	e86a                	sd	s10,16(sp)
    800037ca:	e46e                	sd	s11,8(sp)
    800037cc:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    800037ce:	40000c93          	li	s9,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    800037d2:	5c7d                	li	s8,-1
    800037d4:	a825                	j	8000380c <writei+0x82>
    800037d6:	020d1d93          	slli	s11,s10,0x20
    800037da:	020ddd93          	srli	s11,s11,0x20
    800037de:	05848513          	addi	a0,s1,88
    800037e2:	86ee                	mv	a3,s11
    800037e4:	8652                	mv	a2,s4
    800037e6:	85de                	mv	a1,s7
    800037e8:	953a                	add	a0,a0,a4
    800037ea:	a95fe0ef          	jal	8000227e <either_copyin>
    800037ee:	05850a63          	beq	a0,s8,80003842 <writei+0xb8>
      brelse(bp);
      break;
    }
    log_write(bp);
    800037f2:	8526                	mv	a0,s1
    800037f4:	678000ef          	jal	80003e6c <log_write>
    brelse(bp);
    800037f8:	8526                	mv	a0,s1
    800037fa:	d40ff0ef          	jal	80002d3a <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    800037fe:	013d09bb          	addw	s3,s10,s3
    80003802:	012d093b          	addw	s2,s10,s2
    80003806:	9a6e                	add	s4,s4,s11
    80003808:	0569f063          	bgeu	s3,s6,80003848 <writei+0xbe>
    uint addr = bmap(ip, off/BSIZE);
    8000380c:	00a9559b          	srliw	a1,s2,0xa
    80003810:	8556                	mv	a0,s5
    80003812:	fa4ff0ef          	jal	80002fb6 <bmap>
    80003816:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    8000381a:	c59d                	beqz	a1,80003848 <writei+0xbe>
    bp = bread(ip->dev, addr);
    8000381c:	000aa503          	lw	a0,0(s5)
    80003820:	c12ff0ef          	jal	80002c32 <bread>
    80003824:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003826:	3ff97713          	andi	a4,s2,1023
    8000382a:	40ec87bb          	subw	a5,s9,a4
    8000382e:	413b06bb          	subw	a3,s6,s3
    80003832:	8d3e                	mv	s10,a5
    80003834:	2781                	sext.w	a5,a5
    80003836:	0006861b          	sext.w	a2,a3
    8000383a:	f8f67ee3          	bgeu	a2,a5,800037d6 <writei+0x4c>
    8000383e:	8d36                	mv	s10,a3
    80003840:	bf59                	j	800037d6 <writei+0x4c>
      brelse(bp);
    80003842:	8526                	mv	a0,s1
    80003844:	cf6ff0ef          	jal	80002d3a <brelse>
  }

  if(off > ip->size)
    80003848:	04caa783          	lw	a5,76(s5)
    8000384c:	0327fa63          	bgeu	a5,s2,80003880 <writei+0xf6>
    ip->size = off;
    80003850:	052aa623          	sw	s2,76(s5)
  ...
  iupdate(ip);
    ...
    80003860:	9ebff0ef          	jal	8000324a <iupdate>

  return tot;
    80003864:	0009851b          	sext.w	a0,s3
    80003868:	69a6                	ld	s3,72(sp)
}
    8000386a:	70a6                	ld	ra,104(sp)
    8000386c:	7406                	ld	s0,96(sp)
    8000386e:	6946                	ld	s2,80(sp)
    80003870:	6a06                	ld	s4,64(sp)
    80003872:	7ae2                	ld	s5,56(sp)
    80003874:	7b42                	ld	s6,48(sp)
    80003876:	7ba2                	ld	s7,40(sp)
    80003878:	6165                	addi	sp,sp,112
    8000387a:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    8000387c:	89da                	mv	s3,s6
    8000387e:	b7c5                	j	8000385e <writei+0xd4>
    80003880:	64e6                	ld	s1,88(sp)
    80003882:	7c02                	ld	s8,32(sp)
    80003884:	6ce2                	ld	s9,24(sp)
    80003886:	6d42                	ld	s10,16(sp)
    80003888:	6da2                	ld	s11,8(sp)
    8000388a:	bfd1                	j	8000385e <writei+0xd4>
    return -1;
    ...
    80003892:	bfe1                	j	8000386a <writei+0xe0>
    return -1;
    80003894:	557d                	li	a0,-1
    80003896:	bfd1                	j	8000386a <writei+0xe0>
...
namecmp(const char *s, const char *t)
{
  ...
  return strncmp(s, t, DIRSIZ);
    ...
    800038a2:	cccfd0ef          	jal	80000d6e <strncmp>
}
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
    ...
    800038b6:	f04a                	sd	s2,32(sp)
    800038b8:	ec4e                	sd	s3,24(sp)
    800038ba:	e852                	sd	s4,16(sp)
    800038bc:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    800038be:	04451703          	lh	a4,68(a0)
    800038c2:	4785                	li	a5,1
    800038c4:	00f71a63          	bne	a4,a5,800038d8 <dirlookup+0x2a>
    800038c8:	892a                	mv	s2,a0
    800038ca:	89ae                	mv	s3,a1
  ...
  for(off = 0; off < dp->size; off += sizeof(de)){
    800038d4:	e39d                	bnez	a5,800038fa <dirlookup+0x4c>
    800038d6:	a095                	j	8000393a <dirlookup+0x8c>
    panic("dirlookup not DIR");
    800038d8:	00004517          	auipc	a0,0x4
    800038dc:	bc850513          	addi	a0,a0,-1080 # 800074a0 <etext+0x4a0>
    800038e0:	f01fc0ef          	jal	800007e0 <panic>
      panic("dirlookup read");
    800038e4:	00004517          	auipc	a0,0x4
    800038e8:	bd450513          	addi	a0,a0,-1068 # 800074b8 <etext+0x4b8>
    800038ec:	ef5fc0ef          	jal	800007e0 <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800038f0:	24c1                	addiw	s1,s1,16
    800038f2:	04c92783          	lw	a5,76(s2)
    800038f6:	04f4f163          	bgeu	s1,a5,80003938 <dirlookup+0x8a>
    ...
    80003906:	d89ff0ef          	jal	8000368e <readi>
    8000390a:	47c1                	li	a5,16
    8000390c:	fcf51ce3          	bne	a0,a5,800038e4 <dirlookup+0x36>
    if(de.inum == 0)
    80003910:	fc045783          	lhu	a5,-64(s0)
    80003914:	dff1                	beqz	a5,800038f0 <dirlookup+0x42>
    if(namecmp(name, de.name) == 0){
    80003916:	fc240593          	addi	a1,s0,-62
    8000391a:	854e                	mv	a0,s3
    8000391c:	f7dff0ef          	jal	80003898 <namecmp>
    80003920:	f961                	bnez	a0,800038f0 <dirlookup+0x42>
      if(poff)
    80003922:	000a0463          	beqz	s4,8000392a <dirlookup+0x7c>
        *poff = off;
    80003926:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    8000392a:	fc045583          	lhu	a1,-64(s0)
    8000392e:	00092503          	lw	a0,0(s2)
    80003932:	f58ff0ef          	jal	8000308a <iget>
    80003936:	a011                	j	8000393a <dirlookup+0x8c>
  return 0;
...
namex(char *path, int nameiparent, char *name)
{
    ...
    80003966:	8b2e                	mv	s6,a1
    80003968:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    8000396a:	00054703          	lbu	a4,0(a0)
    8000396e:	02f00793          	li	a5,47
    80003972:	00f70e63          	beq	a4,a5,8000398e <namex+0x44>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    80003976:	f59fd0ef          	jal	800018ce <myproc>
    8000397a:	15053503          	ld	a0,336(a0)
    8000397e:	94bff0ef          	jal	800032c8 <idup>
    80003982:	8a2a                	mv	s4,a0
  while(*path == '/')
    80003984:	02f00913          	li	s2,47
  if(len >= DIRSIZ)
    80003988:	4c35                	li	s8,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    8000398a:	4b85                	li	s7,1
    8000398c:	a871                	j	80003a28 <namex+0xde>
    ip = iget(ROOTDEV, ROOTINO);
    8000398e:	4585                	li	a1,1
    80003990:	4505                	li	a0,1
    80003992:	ef8ff0ef          	jal	8000308a <iget>
    80003996:	8a2a                	mv	s4,a0
    80003998:	b7f5                	j	80003984 <namex+0x3a>
      iunlockput(ip);
    8000399a:	8552                	mv	a0,s4
    8000399c:	b6dff0ef          	jal	80003508 <iunlockput>
      return 0;
  ...
  return ip;
    ...
    800039c0:	9edff0ef          	jal	800033ac <iunlock>
      return ip;
    800039c4:	bff9                	j	800039a2 <namex+0x58>
      iunlockput(ip);
    800039c6:	8552                	mv	a0,s4
    800039c8:	b41ff0ef          	jal	80003508 <iunlockput>
      return 0;
    800039cc:	8a4e                	mv	s4,s3
    800039ce:	bfd1                	j	800039a2 <namex+0x58>
  len = path - s;
    800039d0:	40998633          	sub	a2,s3,s1
    800039d4:	00060c9b          	sext.w	s9,a2
  if(len >= DIRSIZ)
    800039d8:	099c5063          	bge	s8,s9,80003a58 <namex+0x10e>
    memmove(name, s, DIRSIZ);
    800039dc:	4639                	li	a2,14
    800039de:	85a6                	mv	a1,s1
    800039e0:	8556                	mv	a0,s5
    800039e2:	b1cfd0ef          	jal	80000cfe <memmove>
    800039e6:	84ce                	mv	s1,s3
  while(*path == '/')
    800039e8:	0004c783          	lbu	a5,0(s1)
    800039ec:	01279763          	bne	a5,s2,800039fa <namex+0xb0>
    path++;
    800039f0:	0485                	addi	s1,s1,1
  while(*path == '/')
    800039f2:	0004c783          	lbu	a5,0(s1)
    800039f6:	ff278de3          	beq	a5,s2,800039f0 <namex+0xa6>
    ilock(ip);
    800039fa:	8552                	mv	a0,s4
    800039fc:	903ff0ef          	jal	800032fe <ilock>
    if(ip->type != T_DIR){
    80003a00:	044a1783          	lh	a5,68(s4)
    80003a04:	f9779be3          	bne	a5,s7,8000399a <namex+0x50>
    if(nameiparent && *path == '\0'){
    80003a08:	000b0563          	beqz	s6,80003a12 <namex+0xc8>
    80003a0c:	0004c783          	lbu	a5,0(s1)
    80003a10:	d7dd                	beqz	a5,800039be <namex+0x74>
    if((next = dirlookup(ip, name, 0)) == 0){
    80003a12:	4601                	li	a2,0
    80003a14:	85d6                	mv	a1,s5
    80003a16:	8552                	mv	a0,s4
    80003a18:	e97ff0ef          	jal	800038ae <dirlookup>
    80003a1c:	89aa                	mv	s3,a0
    80003a1e:	d545                	beqz	a0,800039c6 <namex+0x7c>
    iunlockput(ip);
    80003a20:	8552                	mv	a0,s4
    80003a22:	ae7ff0ef          	jal	80003508 <iunlockput>
    ip = next;
    80003a26:	8a4e                	mv	s4,s3
  while(*path == '/')
    80003a28:	0004c783          	lbu	a5,0(s1)
    80003a2c:	01279763          	bne	a5,s2,80003a3a <namex+0xf0>
    path++;
    80003a30:	0485                	addi	s1,s1,1
  while(*path == '/')
    80003a32:	0004c783          	lbu	a5,0(s1)
    80003a36:	ff278de3          	beq	a5,s2,80003a30 <namex+0xe6>
  if(*path == 0)
    80003a3a:	cb8d                	beqz	a5,80003a6c <namex+0x122>
  while(*path != '/' && *path != 0)
    80003a3c:	0004c783          	lbu	a5,0(s1)
    80003a40:	89a6                	mv	s3,s1
  len = path - s;
    80003a42:	4c81                	li	s9,0
    80003a44:	4601                	li	a2,0
  while(*path != '/' && *path != 0)
    80003a46:	01278963          	beq	a5,s2,80003a58 <namex+0x10e>
    80003a4a:	d3d9                	beqz	a5,800039d0 <namex+0x86>
    path++;
    80003a4c:	0985                	addi	s3,s3,1
  while(*path != '/' && *path != 0)
    80003a4e:	0009c783          	lbu	a5,0(s3)
    80003a52:	ff279ce3          	bne	a5,s2,80003a4a <namex+0x100>
    80003a56:	bfad                	j	800039d0 <namex+0x86>
    memmove(name, s, len);
    80003a58:	2601                	sext.w	a2,a2
    80003a5a:	85a6                	mv	a1,s1
    80003a5c:	8556                	mv	a0,s5
    80003a5e:	aa0fd0ef          	jal	80000cfe <memmove>
    name[len] = 0;
    80003a62:	9cd6                	add	s9,s9,s5
    80003a64:	000c8023          	sb	zero,0(s9) # 2000 <_entry-0x7fffe000>
    80003a68:	84ce                	mv	s1,s3
    80003a6a:	bfbd                	j	800039e8 <namex+0x9e>
  if(nameiparent){
    80003a6c:	f20b0be3          	beqz	s6,800039a2 <namex+0x58>
    iput(ip);
    80003a70:	8552                	mv	a0,s4
    80003a72:	a0fff0ef          	jal	80003480 <iput>
    return 0;
    80003a76:	4a01                	li	s4,0
    80003a78:	b72d                	j	800039a2 <namex+0x58>

0000000080003a7a <dirlink>:
{
    80003a7a:	7139                	addi	sp,sp,-64
    80003a7c:	fc06                	sd	ra,56(sp)
    80003a7e:	f822                	sd	s0,48(sp)
    ...
    80003a8c:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    80003a8e:	4601                	li	a2,0
    80003a90:	e1fff0ef          	jal	800038ae <dirlookup>
    80003a94:	e535                	bnez	a0,80003b00 <dirlink+0x86>
    80003a96:	f426                	sd	s1,40(sp)
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003a98:	04c92483          	lw	s1,76(s2)
    80003a9c:	c48d                	beqz	s1,80003ac6 <dirlink+0x4c>
    ...
    80003aac:	be3ff0ef          	jal	8000368e <readi>
    80003ab0:	47c1                	li	a5,16
    80003ab2:	04f51b63          	bne	a0,a5,80003b08 <dirlink+0x8e>
    if(de.inum == 0)
    80003ab6:	fc045783          	lhu	a5,-64(s0)
    80003aba:	c791                	beqz	a5,80003ac6 <dirlink+0x4c>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003abc:	24c1                	addiw	s1,s1,16
    80003abe:	04c92783          	lw	a5,76(s2)
    80003ac2:	fcf4efe3          	bltu	s1,a5,80003aa0 <dirlink+0x26>
  strncpy(de.name, name, DIRSIZ);
    80003ac6:	4639                	li	a2,14
    80003ac8:	85d2                	mv	a1,s4
    80003aca:	fc240513          	addi	a0,s0,-62
    80003ace:	ad6fd0ef          	jal	80000da4 <strncpy>
  de.inum = inum;
    80003ad2:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    ...
    80003ae2:	ca9ff0ef          	jal	8000378a <writei>
    80003ae6:	1541                	addi	a0,a0,-16
    80003ae8:	00a03533          	snez	a0,a0
    80003aec:	40a00533          	neg	a0,a0
    80003af0:	74a2                	ld	s1,40(sp)
}
    80003af2:	70e2                	ld	ra,56(sp)
    80003af4:	7442                	ld	s0,48(sp)
    80003af6:	7902                	ld	s2,32(sp)
    80003af8:	69e2                	ld	s3,24(sp)
    80003afa:	6a42                	ld	s4,16(sp)
    80003afc:	6121                	addi	sp,sp,64
    80003afe:	8082                	ret
    iput(ip);
    80003b00:	981ff0ef          	jal	80003480 <iput>
    return -1;
    80003b04:	557d                	li	a0,-1
    80003b06:	b7f5                	j	80003af2 <dirlink+0x78>
      panic("dirlink read");
    80003b08:	00004517          	auipc	a0,0x4
    80003b0c:	9c050513          	addi	a0,a0,-1600 # 800074c8 <etext+0x4c8>
    80003b10:	cd1fc0ef          	jal	800007e0 <panic>

0000000080003b14 <namei>:
...
namei(char *path)
{
  ...
  return namex(path, 0, name);
    80003b1c:	fe040613          	addi	a2,s0,-32
    80003b20:	4581                	li	a1,0
    80003b22:	e29ff0ef          	jal	8000394a <namex>
}
...
nameiparent(char *path, char *name)
{
    ...
    80003b36:	862e                	mv	a2,a1
  return namex(path, 1, name);
    80003b38:	4585                	li	a1,1
    80003b3a:	e11ff0ef          	jal	8000394a <namex>
}
...
write_head(void)
{
  ...
  struct buf *buf = bread(log.dev, log.start);
    80003b52:	0001c917          	auipc	s2,0x1c
    80003b56:	de690913          	addi	s2,s2,-538 # 8001f938 <log>
    80003b5a:	01892583          	lw	a1,24(s2)
    80003b5e:	02492503          	lw	a0,36(s2)
    80003b62:	8d0ff0ef          	jal	80002c32 <bread>
    80003b66:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    80003b68:	02892603          	lw	a2,40(s2)
    80003b6c:	cd30                	sw	a2,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    80003b6e:	00c05f63          	blez	a2,80003b8c <write_head+0x46>
    80003b72:	0001c717          	auipc	a4,0x1c
    80003b76:	df270713          	addi	a4,a4,-526 # 8001f964 <log+0x2c>
    80003b7a:	87aa                	mv	a5,a0
    80003b7c:	060a                	slli	a2,a2,0x2
    80003b7e:	962a                	add	a2,a2,a0
    hb->block[i] = log.lh.block[i];
    80003b80:	4314                	lw	a3,0(a4)
    80003b82:	cff4                	sw	a3,92(a5)
  for (i = 0; i < log.lh.n; i++) {
    80003b84:	0711                	addi	a4,a4,4
    80003b86:	0791                	addi	a5,a5,4
    80003b88:	fec79ce3          	bne	a5,a2,80003b80 <write_head+0x3a>
  }
  bwrite(buf);
    80003b8c:	8526                	mv	a0,s1
    80003b8e:	97aff0ef          	jal	80002d08 <bwrite>
  brelse(buf);
    80003b92:	8526                	mv	a0,s1
    80003b94:	9a6ff0ef          	jal	80002d3a <brelse>
}
...
0000000080003ba4 <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    80003ba4:	0001c797          	auipc	a5,0x1c
    80003ba8:	dbc7a783          	lw	a5,-580(a5) # 8001f960 <log+0x28>
    80003bac:	0af05e63          	blez	a5,80003c68 <install_trans+0xc4>
{
    ...
    80003bc6:	8b2a                	mv	s6,a0
    80003bc8:	0001ca97          	auipc	s5,0x1c
    80003bcc:	d9ca8a93          	addi	s5,s5,-612 # 8001f964 <log+0x2c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003bd0:	4981                	li	s3,0
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003bd2:	00004b97          	auipc	s7,0x4
    80003bd6:	906b8b93          	addi	s7,s7,-1786 # 800074d8 <etext+0x4d8>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003bda:	0001ca17          	auipc	s4,0x1c
    80003bde:	d5ea0a13          	addi	s4,s4,-674 # 8001f938 <log>
    80003be2:	a025                	j	80003c0a <install_trans+0x66>
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003be4:	000aa603          	lw	a2,0(s5)
    80003be8:	85ce                	mv	a1,s3
    80003bea:	855e                	mv	a0,s7
    80003bec:	90ffc0ef          	jal	800004fa <printf>
    80003bf0:	a839                	j	80003c0e <install_trans+0x6a>
    brelse(lbuf);
    80003bf2:	854a                	mv	a0,s2
    80003bf4:	946ff0ef          	jal	80002d3a <brelse>
    brelse(dbuf);
    80003bf8:	8526                	mv	a0,s1
    80003bfa:	940ff0ef          	jal	80002d3a <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003bfe:	2985                	addiw	s3,s3,1
    80003c00:	0a91                	addi	s5,s5,4
    80003c02:	028a2783          	lw	a5,40(s4)
    80003c06:	04f9d663          	bge	s3,a5,80003c52 <install_trans+0xae>
    if(recovering) {
    80003c0a:	fc0b1de3          	bnez	s6,80003be4 <install_trans+0x40>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003c0e:	018a2583          	lw	a1,24(s4)
    80003c12:	013585bb          	addw	a1,a1,s3
    80003c16:	2585                	addiw	a1,a1,1
    80003c18:	024a2503          	lw	a0,36(s4)
    80003c1c:	816ff0ef          	jal	80002c32 <bread>
    80003c20:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80003c22:	000aa583          	lw	a1,0(s5)
    80003c26:	024a2503          	lw	a0,36(s4)
    80003c2a:	808ff0ef          	jal	80002c32 <bread>
    80003c2e:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80003c30:	40000613          	li	a2,1024
    80003c34:	05890593          	addi	a1,s2,88
    80003c38:	05850513          	addi	a0,a0,88
    80003c3c:	8c2fd0ef          	jal	80000cfe <memmove>
    bwrite(dbuf);  // write dst to disk
    80003c40:	8526                	mv	a0,s1
    80003c42:	8c6ff0ef          	jal	80002d08 <bwrite>
    if(recovering == 0)
    80003c46:	fa0b16e3          	bnez	s6,80003bf2 <install_trans+0x4e>
      bunpin(dbuf);
    80003c4a:	8526                	mv	a0,s1
    80003c4c:	9aaff0ef          	jal	80002df6 <bunpin>
    80003c50:	b74d                	j	80003bf2 <install_trans+0x4e>
}
    ...
    80003c5c:	7a02                	ld	s4,32(sp)
    80003c5e:	6ae2                	ld	s5,24(sp)
    80003c60:	6b42                	ld	s6,16(sp)
    80003c62:	6ba2                	ld	s7,8(sp)
    80003c64:	6161                	addi	sp,sp,80
    80003c66:	8082                	ret
    80003c68:	8082                	ret
...
0000000080003c6a <initlog>:
{
  ...
  initlock(&log.lock, "log");
    ...
    80003c80:	cbc48493          	addi	s1,s1,-836 # 8001f938 <log>
    80003c84:	00004597          	auipc	a1,0x4
    80003c88:	87458593          	addi	a1,a1,-1932 # 800074f8 <etext+0x4f8>
    80003c8c:	8526                	mv	a0,s1
    80003c8e:	ec1fc0ef          	jal	80000b4e <initlock>
  log.start = sb->logstart;
    80003c92:	0149a583          	lw	a1,20(s3)
    80003c96:	cc8c                	sw	a1,24(s1)
  log.dev = dev;
    80003c98:	0324a223          	sw	s2,36(s1)
  struct buf *buf = bread(log.dev, log.start);
    80003c9c:	854a                	mv	a0,s2
    80003c9e:	f95fe0ef          	jal	80002c32 <bread>
  log.lh.n = lh->n;
    80003ca2:	4d30                	lw	a2,88(a0)
    80003ca4:	d490                	sw	a2,40(s1)
  for (i = 0; i < log.lh.n; i++) {
    80003ca6:	00c05f63          	blez	a2,80003cc4 <initlog+0x5a>
    80003caa:	87aa                	mv	a5,a0
    80003cac:	0001c717          	auipc	a4,0x1c
    80003cb0:	cb870713          	addi	a4,a4,-840 # 8001f964 <log+0x2c>
    80003cb4:	060a                	slli	a2,a2,0x2
    80003cb6:	962a                	add	a2,a2,a0
    log.lh.block[i] = lh->block[i];
    80003cb8:	4ff4                	lw	a3,92(a5)
    80003cba:	c314                	sw	a3,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80003cbc:	0791                	addi	a5,a5,4
    80003cbe:	0711                	addi	a4,a4,4
    80003cc0:	fec79ce3          	bne	a5,a2,80003cb8 <initlog+0x4e>
  brelse(buf);
    80003cc4:	876ff0ef          	jal	80002d3a <brelse>
...
recover_from_log(void)
{
  ...
  install_trans(1); // if committed, copy from log to disk
    ...
    80003cca:	edbff0ef          	jal	80003ba4 <install_trans>
  log.lh.n = 0;
    80003cce:	0001c797          	auipc	a5,0x1c
    80003cd2:	c807a923          	sw	zero,-878(a5) # 8001f960 <log+0x28>
  write_head(); // clear the log
    80003cd6:	e71ff0ef          	jal	80003b46 <write_head>
}
...
begin_op(void)
{
  ...
  acquire(&log.lock);
    ...
    80003cf8:	c4450513          	addi	a0,a0,-956 # 8001f938 <log>
    80003cfc:	ed3fc0ef          	jal	80000bce <acquire>
  while(1){
    if(log.committing){
    80003d00:	0001c497          	auipc	s1,0x1c
    80003d04:	c3848493          	addi	s1,s1,-968 # 8001f938 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003d08:	4979                	li	s2,30
    80003d0a:	a029                	j	80003d14 <begin_op+0x2c>
      sleep(&log, &log.lock);
    80003d0c:	85a6                	mv	a1,s1
    80003d0e:	8526                	mv	a0,s1
    80003d10:	9c8fe0ef          	jal	80001ed8 <sleep>
    if(log.committing){
    80003d14:	509c                	lw	a5,32(s1)
    80003d16:	fbfd                	bnez	a5,80003d0c <begin_op+0x24>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003d18:	4cd8                	lw	a4,28(s1)
    80003d1a:	2705                	addiw	a4,a4,1
    80003d1c:	0027179b          	slliw	a5,a4,0x2
    80003d20:	9fb9                	addw	a5,a5,a4
    80003d22:	0017979b          	slliw	a5,a5,0x1
    80003d26:	5494                	lw	a3,40(s1)
    80003d28:	9fb5                	addw	a5,a5,a3
    80003d2a:	00f95763          	bge	s2,a5,80003d38 <begin_op+0x50>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    80003d2e:	85a6                	mv	a1,s1
    80003d30:	8526                	mv	a0,s1
    80003d32:	9a6fe0ef          	jal	80001ed8 <sleep>
    80003d36:	bff9                	j	80003d14 <begin_op+0x2c>
    } else {
      log.outstanding += 1;
    80003d38:	0001c517          	auipc	a0,0x1c
    80003d3c:	c0050513          	addi	a0,a0,-1024 # 8001f938 <log>
    80003d40:	cd58                	sw	a4,28(a0)
      release(&log.lock);
    80003d42:	f25fc0ef          	jal	80000c66 <release>
      break;
    }
  }
}
...
end_op(void)
{
  ...
  acquire(&log.lock);
    ...
    80003d62:	bda48493          	addi	s1,s1,-1062 # 8001f938 <log>
    80003d66:	8526                	mv	a0,s1
    80003d68:	e67fc0ef          	jal	80000bce <acquire>
  log.outstanding -= 1;
    80003d6c:	4cdc                	lw	a5,28(s1)
    80003d6e:	37fd                	addiw	a5,a5,-1
    80003d70:	0007891b          	sext.w	s2,a5
    80003d74:	ccdc                	sw	a5,28(s1)
  if(log.committing)
    80003d76:	509c                	lw	a5,32(s1)
    80003d78:	ef9d                	bnez	a5,80003db6 <end_op+0x64>
    panic("log.committing");
  if(log.outstanding == 0){
    80003d7a:	04091763          	bnez	s2,80003dc8 <end_op+0x76>
    do_commit = 1;
    log.committing = 1;
    80003d7e:	0001c497          	auipc	s1,0x1c
    80003d82:	bba48493          	addi	s1,s1,-1094 # 8001f938 <log>
  ...
  release(&log.lock);
    ...
    80003d8c:	edbfc0ef          	jal	80000c66 <release>
}
...
commit()
{
  if (log.lh.n > 0) {
    ...
    80003d92:	04f04b63          	bgtz	a5,80003de8 <end_op+0x96>
    acquire(&log.lock);
    80003d96:	0001c497          	auipc	s1,0x1c
    80003d9a:	ba248493          	addi	s1,s1,-1118 # 8001f938 <log>
    80003d9e:	8526                	mv	a0,s1
    80003da0:	e2ffc0ef          	jal	80000bce <acquire>
    log.committing = 0;
    80003da4:	0204a023          	sw	zero,32(s1)
    wakeup(&log);
    80003da8:	8526                	mv	a0,s1
    80003daa:	97afe0ef          	jal	80001f24 <wakeup>
    release(&log.lock);
    80003dae:	8526                	mv	a0,s1
    80003db0:	eb7fc0ef          	jal	80000c66 <release>
}
    80003db4:	a025                	j	80003ddc <end_op+0x8a>
    80003db6:	ec4e                	sd	s3,24(sp)
    80003db8:	e852                	sd	s4,16(sp)
    80003dba:	e456                	sd	s5,8(sp)
    panic("log.committing");
    80003dbc:	00003517          	auipc	a0,0x3
    80003dc0:	74450513          	addi	a0,a0,1860 # 80007500 <etext+0x500>
    80003dc4:	a1dfc0ef          	jal	800007e0 <panic>
    wakeup(&log);
    80003dc8:	0001c497          	auipc	s1,0x1c
    80003dcc:	b7048493          	addi	s1,s1,-1168 # 8001f938 <log>
    80003dd0:	8526                	mv	a0,s1
    80003dd2:	952fe0ef          	jal	80001f24 <wakeup>
  release(&log.lock);
    80003dd6:	8526                	mv	a0,s1
    80003dd8:	e8ffc0ef          	jal	80000c66 <release>
}
    ...
    80003dec:	e456                	sd	s5,8(sp)
  for (tail = 0; tail < log.lh.n; tail++) {
    80003dee:	0001ca97          	auipc	s5,0x1c
    80003df2:	b76a8a93          	addi	s5,s5,-1162 # 8001f964 <log+0x2c>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80003df6:	0001ca17          	auipc	s4,0x1c
    80003dfa:	b42a0a13          	addi	s4,s4,-1214 # 8001f938 <log>
    80003dfe:	018a2583          	lw	a1,24(s4)
    80003e02:	012585bb          	addw	a1,a1,s2
    80003e06:	2585                	addiw	a1,a1,1
    80003e08:	024a2503          	lw	a0,36(s4)
    80003e0c:	e27fe0ef          	jal	80002c32 <bread>
    80003e10:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    80003e12:	000aa583          	lw	a1,0(s5)
    80003e16:	024a2503          	lw	a0,36(s4)
    80003e1a:	e19fe0ef          	jal	80002c32 <bread>
    80003e1e:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    80003e20:	40000613          	li	a2,1024
    80003e24:	05850593          	addi	a1,a0,88
    80003e28:	05848513          	addi	a0,s1,88
    80003e2c:	ed3fc0ef          	jal	80000cfe <memmove>
    bwrite(to);  // write the log
    80003e30:	8526                	mv	a0,s1
    80003e32:	ed7fe0ef          	jal	80002d08 <bwrite>
    brelse(from);
    80003e36:	854e                	mv	a0,s3
    80003e38:	f03fe0ef          	jal	80002d3a <brelse>
    brelse(to);
    80003e3c:	8526                	mv	a0,s1
    80003e3e:	efdfe0ef          	jal	80002d3a <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003e42:	2905                	addiw	s2,s2,1
    80003e44:	0a91                	addi	s5,s5,4
    80003e46:	028a2783          	lw	a5,40(s4)
    80003e4a:	faf94ae3          	blt	s2,a5,80003dfe <end_op+0xac>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    80003e4e:	cf9ff0ef          	jal	80003b46 <write_head>
    install_trans(0); // Now install writes to home locations
    80003e52:	4501                	li	a0,0
    80003e54:	d51ff0ef          	jal	80003ba4 <install_trans>
    log.lh.n = 0;
    80003e58:	0001c797          	auipc	a5,0x1c
    80003e5c:	b007a423          	sw	zero,-1272(a5) # 8001f960 <log+0x28>
    write_head();    // Erase the transaction from the log
    80003e60:	ce7ff0ef          	jal	80003b46 <write_head>
    80003e64:	69e2                	ld	s3,24(sp)
    80003e66:	6a42                	ld	s4,16(sp)
    80003e68:	6aa2                	ld	s5,8(sp)
    80003e6a:	b735                	j	80003d96 <end_op+0x44>

0000000080003e6c <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
  ...
  acquire(&log.lock);
    80003e7a:	0001c917          	auipc	s2,0x1c
    80003e7e:	abe90913          	addi	s2,s2,-1346 # 8001f938 <log>
    80003e82:	854a                	mv	a0,s2
    80003e84:	d4bfc0ef          	jal	80000bce <acquire>
  if (log.lh.n >= LOGBLOCKS)
    80003e88:	02892603          	lw	a2,40(s2)
    80003e8c:	47f5                	li	a5,29
    80003e8e:	04c7cc63          	blt	a5,a2,80003ee6 <log_write+0x7a>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80003e92:	0001c797          	auipc	a5,0x1c
    80003e96:	ac27a783          	lw	a5,-1342(a5) # 8001f954 <log+0x1c>
    80003e9a:	04f05c63          	blez	a5,80003ef2 <log_write+0x86>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80003e9e:	4781                	li	a5,0
    80003ea0:	04c05f63          	blez	a2,80003efe <log_write+0x92>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003ea4:	44cc                	lw	a1,12(s1)
    80003ea6:	0001c717          	auipc	a4,0x1c
    80003eaa:	abe70713          	addi	a4,a4,-1346 # 8001f964 <log+0x2c>
  for (i = 0; i < log.lh.n; i++) {
    80003eae:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003eb0:	4314                	lw	a3,0(a4)
    80003eb2:	04b68663          	beq	a3,a1,80003efe <log_write+0x92>
  for (i = 0; i < log.lh.n; i++) {
    80003eb6:	2785                	addiw	a5,a5,1
    80003eb8:	0711                	addi	a4,a4,4
    80003eba:	fef61be3          	bne	a2,a5,80003eb0 <log_write+0x44>
      break;
  }
  log.lh.block[i] = b->blockno;
    ...
    80003ec6:	a7678793          	addi	a5,a5,-1418 # 8001f938 <log>
    80003eca:	97b2                	add	a5,a5,a2
    80003ecc:	44d8                	lw	a4,12(s1)
    80003ece:	c7d8                	sw	a4,12(a5)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    80003ed0:	8526                	mv	a0,s1
    80003ed2:	ef1fe0ef          	jal	80002dc2 <bpin>
    log.lh.n++;
    80003ed6:	0001c717          	auipc	a4,0x1c
    80003eda:	a6270713          	addi	a4,a4,-1438 # 8001f938 <log>
    80003ede:	571c                	lw	a5,40(a4)
    80003ee0:	2785                	addiw	a5,a5,1
    80003ee2:	d71c                	sw	a5,40(a4)
    80003ee4:	a80d                	j	80003f16 <log_write+0xaa>
    panic("too big a transaction");
    80003ee6:	00003517          	auipc	a0,0x3
    80003eea:	62a50513          	addi	a0,a0,1578 # 80007510 <etext+0x510>
    80003eee:	8f3fc0ef          	jal	800007e0 <panic>
    panic("log_write outside of trans");
    80003ef2:	00003517          	auipc	a0,0x3
    80003ef6:	63650513          	addi	a0,a0,1590 # 80007528 <etext+0x528>
    80003efa:	8e7fc0ef          	jal	800007e0 <panic>
  log.lh.block[i] = b->blockno;
    80003efe:	00878693          	addi	a3,a5,8
    80003f02:	068a                	slli	a3,a3,0x2
    80003f04:	0001c717          	auipc	a4,0x1c
    80003f08:	a3470713          	addi	a4,a4,-1484 # 8001f938 <log>
    80003f0c:	9736                	add	a4,a4,a3
    80003f0e:	44d4                	lw	a3,12(s1)
    80003f10:	c754                	sw	a3,12(a4)
  if (i == log.lh.n) {  // Add new block to log?
    80003f12:	faf60fe3          	beq	a2,a5,80003ed0 <log_write+0x64>
  }
  release(&log.lock);
    80003f16:	0001c517          	auipc	a0,0x1c
    80003f1a:	a2250513          	addi	a0,a0,-1502 # 8001f938 <log>
    80003f1e:	d49fc0ef          	jal	80000c66 <release>
}
...
initsleeplock(struct sleeplock *lk, char *name)
{
  ...
  initlock(&lk->lk, "sleep lock");
    ...
    80003f42:	60a58593          	addi	a1,a1,1546 # 80007548 <etext+0x548>
    80003f46:	0521                	addi	a0,a0,8
    80003f48:	c07fc0ef          	jal	80000b4e <initlock>
  lk->name = name;
...
acquiresleep(struct sleeplock *lk)
{
  ...
  acquire(&lk->lk);
    ...
    80003f78:	c57fc0ef          	jal	80000bce <acquire>
  while (lk->locked) {
    ...
    80003f84:	f55fd0ef          	jal	80001ed8 <sleep>
  while (lk->locked) {
  ...
  lk->pid = myproc()->pid;
    80003f90:	93ffd0ef          	jal	800018ce <myproc>
...
releasesleep(struct sleeplock *lk)
{
  ...
  acquire(&lk->lk);
    ...
    80003fbe:	c11fc0ef          	jal	80000bce <acquire>
  lk->locked = 0;
  ...
  wakeup(lk);
    ...
    80003fcc:	f59fd0ef          	jal	80001f24 <wakeup>
  release(&lk->lk);
...
holdingsleep(struct sleeplock *lk)
{
  ...
  acquire(&lk->lk);
    ...
    80003ff6:	bd9fc0ef          	jal	80000bce <acquire>
  ...
  r = lk->locked && (lk->pid == myproc()->pid);
    ...
    8000401a:	8b5fd0ef          	jal	800018ce <myproc>
...
fileinit(void)
{
  ...
  initlock(&ftable.lock, "ftable");
    ...
    80004038:	52458593          	addi	a1,a1,1316 # 80007558 <etext+0x558>
    8000403c:	0001c517          	auipc	a0,0x1c
    80004040:	a4450513          	addi	a0,a0,-1468 # 8001fa80 <ftable>
    80004044:	b0bfc0ef          	jal	80000b4e <initlock>
}
...
filealloc(void)
{
  ...
  acquire(&ftable.lock);
    ...
    80004062:	b6dfc0ef          	jal	80000bce <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80004066:	0001c497          	auipc	s1,0x1c
    8000406a:	a3248493          	addi	s1,s1,-1486 # 8001fa98 <ftable+0x18>
    8000406e:	0001d717          	auipc	a4,0x1d
    80004072:	9ca70713          	addi	a4,a4,-1590 # 80020a38 <disk>
    if(f->ref == 0){
  ...
  release(&ftable.lock);
    ...
    80004086:	9fe50513          	addi	a0,a0,-1538 # 8001fa80 <ftable>
  ...
  return 0;
    ...
    8000409a:	9ea50513          	addi	a0,a0,-1558 # 8001fa80 <ftable>
...
filedup(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    800040be:	9c650513          	addi	a0,a0,-1594 # 8001fa80 <ftable>
  ...
  release(&ftable.lock);
    ...
    800040d4:	9b050513          	addi	a0,a0,-1616 # 8001fa80 <ftable>
  ...
  return f;
    ...
    800040ec:	47850513          	addi	a0,a0,1144 # 80007560 <etext+0x560>
...
fileclose(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    80004104:	98050513          	addi	a0,a0,-1664 # 8001fa80 <ftable>
    80004108:	ac7fc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
  ...
  release(&ftable.lock);
    ...
    80004142:	94250513          	addi	a0,a0,-1726 # 8001fa80 <ftable>
    80004146:	b21fc0ef          	jal	80000c66 <release>
  ...
  if(ff.type == FD_PIPE){
    ...
    8000416e:	3fe50513          	addi	a0,a0,1022 # 80007568 <etext+0x568>
    80004172:	e6efc0ef          	jal	800007e0 <panic>
    release(&ftable.lock);
    80004176:	0001c517          	auipc	a0,0x1c
    8000417a:	90a50513          	addi	a0,a0,-1782 # 8001fa80 <ftable>
    8000417e:	ae9fc0ef          	jal	80000c66 <release>
    begin_op();
    iput(ff.ip);
    end_op();
  }
    ...
    begin_op();
    8000419e:	b4bff0ef          	jal	80003ce8 <begin_op>
    iput(ff.ip);
    800041a2:	854e                	mv	a0,s3
    800041a4:	adcff0ef          	jal	80003480 <iput>
    end_op();
    800041a8:	babff0ef          	jal	80003d52 <end_op>
...
filestat(struct file *f, uint64 addr)
{
  ...
  struct proc *p = myproc();
    800041c6:	f08fd0ef          	jal	800018ce <myproc>
  struct stat st;
...
fileread(struct file *f, uint64 addr, int n)
{
  ...
  if(f->type == FD_PIPE){
    ...
    80004244:	8baff0ef          	jal	800032fe <ilock>
    ...
    80004252:	c3cff0ef          	jal	8000368e <readi>
    ...
    80004264:	948ff0ef          	jal	800033ac <iunlock>
  ...
  return r;
    ...
    80004298:	0001b717          	auipc	a4,0x1b
    8000429c:	74870713          	addi	a4,a4,1864 # 8001f9e0 <devsw>
    ...
    800042b6:	2c650513          	addi	a0,a0,710 # 80007578 <etext+0x578>
    800042ba:	d26fc0ef          	jal	800007e0 <panic>
    return -1;
    ...
    800042d6:	10078b63          	beqz	a5,800043ec <filewrite+0x11a>
{
  ...
  if(f->type == FD_PIPE){
    ...
    8000433a:	0001b717          	auipc	a4,0x1b
    8000433e:	6a670713          	addi	a4,a4,1702 # 8001f9e0 <devsw>
    ...
    8000434e:	00048a9b          	sext.w	s5,s1
      ...
      begin_op();
    80004352:	997ff0ef          	jal	80003ce8 <begin_op>
      ilock(f->ip);
    80004356:	01893503          	ld	a0,24(s2)
    8000435a:	fa5fe0ef          	jal	800032fe <ilock>
    ...
    8000436e:	c1cff0ef          	jal	8000378a <writei>
    ...
    80004386:	826ff0ef          	jal	800033ac <iunlock>
      end_op();
    8000438a:	9c9ff0ef          	jal	80003d52 <end_op>
  ...
  return ret;
    ...
    800043e4:	1a850513          	addi	a0,a0,424 # 80007588 <etext+0x588>
    800043e8:	bf8fc0ef          	jal	800007e0 <panic>
    return -1;
...
pipealloc(struct file **f0, struct file **f1)
{
  ...
  *f0 = *f1 = 0;
  ...
  if((pi = (struct pipe*)kalloc()) == 0)
    8000442a:	ed4fc0ef          	jal	80000afe <kalloc>
  ...
  initlock(&pi->lock, "pipe");
    ...
    8000444a:	15258593          	addi	a1,a1,338 # 80007598 <etext+0x598>
    8000444e:	f00fc0ef          	jal	80000b4e <initlock>
  (*f0)->type = FD_PIPE;
...
pipeclose(struct pipe *pi, int writable)
{
  ...
  if(writable){
    ...
    800044e6:	a3ffd0ef          	jal	80001f24 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    ...
    800044f8:	d24fc0ef          	jal	80000a1c <kfree>
  } else
    release(&pi->lock);
}
    ...
    80004510:	a15fd0ef          	jal	80001f24 <wakeup>
    80004514:	bfd9                	j	800044ea <pipeclose+0x24>
    release(&pi->lock);
    80004516:	8526                	mv	a0,s1
    80004518:	f4efc0ef          	jal	80000c66 <release>
}
...
pipewrite(struct pipe *pi, uint64 addr, int n)
{
  ...
  struct proc *pr = myproc();
    80004536:	b98fd0ef          	jal	800018ce <myproc>
    8000453a:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    8000453c:	8526                	mv	a0,s1
    8000453e:	e90fc0ef          	jal	80000bce <acquire>
  while(i < n){
  ...
  return i;
    ...
    8000457e:	9a7fd0ef          	jal	80001f24 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    80004582:	85a6                	mv	a1,s1
    80004584:	855e                	mv	a0,s7
    80004586:	953fd0ef          	jal	80001ed8 <sleep>
  while(i < n){
    ...
    80004596:	b7bfd0ef          	jal	80002110 <killed>
    ...
    800045ba:	90cfd0ef          	jal	800016c6 <copyin>
  ...
  wakeup(&pi->nread);
    ...
    800045ea:	93bfd0ef          	jal	80001f24 <wakeup>
  release(&pi->lock);
...
piperead(struct pipe *pi, uint64 addr, int n)
{
  ...
  int i;
  struct proc *pr = myproc();
    8000461a:	ab4fd0ef          	jal	800018ce <myproc>
    8000461e:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004620:	8526                	mv	a0,s1
    80004622:	dacfc0ef          	jal	80000bce <acquire>
  ...
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    ...
    80004662:	e04fc0ef          	jal	80000c66 <release>
      return -1;
    80004666:	59fd                	li	s3,-1
    80004668:	a8b9                	j	800046c6 <piperead+0xc4>
  ...
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004670:	05505363          	blez	s5,800046b6 <piperead+0xb4>
    if(pi->nread == pi->nwrite)
    80004674:	2184a783          	lw	a5,536(s1)
    80004678:	21c4a703          	lw	a4,540(s1)
    8000467c:	02f70d63          	beq	a4,a5,800046b6 <piperead+0xb4>
    ch = pi->data[pi->nread % PIPESIZE];
    80004680:	1ff7f793          	andi	a5,a5,511
    80004684:	97a6                	add	a5,a5,s1
    80004686:	0187c783          	lbu	a5,24(a5)
    8000468a:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1) {
    8000468e:	4685                	li	a3,1
    80004690:	fbf40613          	addi	a2,s0,-65
    80004694:	85ca                	mv	a1,s2
    80004696:	050a3503          	ld	a0,80(s4)
    8000469a:	f49fc0ef          	jal	800015e2 <copyout>
    8000469e:	03650e63          	beq	a0,s6,800046da <piperead+0xd8>
      if(i == 0)
        i = -1;
      break;
    }
    ...
    800046a8:	20f4ac23          	sw	a5,536(s1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800046ac:	2985                	addiw	s3,s3,1
    800046ae:	0905                	addi	s2,s2,1
    800046b0:	fd3a92e3          	bne	s5,s3,80004674 <piperead+0x72>
    800046b4:	89d6                	mv	s3,s5
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    800046b6:	21c48513          	addi	a0,s1,540
    800046ba:	86bfd0ef          	jal	80001f24 <wakeup>
  release(&pi->lock);
    800046be:	8526                	mv	a0,s1
    800046c0:	da6fc0ef          	jal	80000c66 <release>
  ...
  return i;
    ...
    800046d2:	7a02                	ld	s4,32(sp)
    800046d4:	6ae2                	ld	s5,24(sp)
    800046d6:	6161                	addi	sp,sp,80
    800046d8:	8082                	ret
      if(i == 0)
    800046da:	fc099ee3          	bnez	s3,800046b6 <piperead+0xb4>
        i = -1;
    800046de:	89aa                	mv	s3,a0
    800046e0:	bfd9                	j	800046b6 <piperead+0xb4>
...
kexec(char *path, char **argv)
{
  ...
  begin_op();
    8000471e:	dcaff0ef          	jal	80003ce8 <begin_op>

  // Open the executable file.
  if((ip = namei(path)) == 0){
    80004722:	854a                	mv	a0,s2
    80004724:	bf0ff0ef          	jal	80003b14 <namei>
  ...
  ilock(ip);
    8000472e:	bd1fe0ef          	jal	800032fe <ilock>
  ...
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    ...
    80004740:	f4ffe0ef          	jal	8000368e <readi>
 ...
 bad:
  ...
  if(ip){
    ...
    8000475e:	dabfe0ef          	jal	80003508 <iunlockput>
    end_op();
    80004762:	df0ff0ef          	jal	80003d52 <end_op>
  }
  return -1;
    ...
    8000477a:	8082                	ret
    end_op();
    8000477c:	dd6ff0ef          	jal	80003d52 <end_op>
    return -1;
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    800047d6:	eb9fe0ef          	jal	8000368e <readi>
  ...
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    ...
    8000482c:	e63fe0ef          	jal	8000368e <readi>
  ...
  iunlockput(ip);
    ...
    8000489c:	c6dfe0ef          	jal	80003508 <iunlockput>
  end_op();
    800048a0:	cb2ff0ef          	jal	80003d52 <end_op>
  p = myproc();
  ...
  ustack[argc] = 0;
    ...
    80004958:	f9078793          	addi	a5,a5,-112 # ffffffffffffef90 <end+0xffffffff7ffde418>
...
argfd(int n, int *pfd, struct file **pf)
{
  ...
  argint(n, &fd);
    ...
    80004a82:	d5bfd0ef          	jal	800027dc <argint>
...
create(char *path, short type, short major, short minor)
{
  ...
  return 0;
    ...
    80004b76:	993fe0ef          	jal	80003508 <iunlockput>
    return 0;
  ...
  if((ip = ialloc(dp->dev, type)) == 0){
    ...
    80004b84:	e0afe0ef          	jal	8000318e <ialloc>
    80004b88:	8a2a                	mv	s4,a0
    80004b8a:	cd15                	beqz	a0,80004bc6 <create+0xc2>
  ilock(ip);
    80004b8c:	f72fe0ef          	jal	800032fe <ilock>
  ip->major = major;
  ...
  iupdate(ip);
    ...
    80004ba0:	eaafe0ef          	jal	8000324a <iupdate>
  ...
  if(dirlink(dp, name, ip->inum) < 0)
    ...
    80004bb2:	ec9fe0ef          	jal	80003a7a <dirlink>
    80004bb6:	06054563          	bltz	a0,80004c20 <create+0x11c>
  iunlockput(dp);
    80004bba:	8526                	mv	a0,s1
    80004bbc:	94dfe0ef          	jal	80003508 <iunlockput>
  return ip;
    ...
    80004bc8:	941fe0ef          	jal	80003508 <iunlockput>
    return 0;
    ...
    80004be0:	e9bfe0ef          	jal	80003a7a <dirlink>
    ...
    80004bf4:	e87fe0ef          	jal	80003a7a <dirlink>
  ...
  if(dirlink(dp, name, ip->inum) < 0)
    ...
    80004c06:	e75fe0ef          	jal	80003a7a <dirlink>
    ...
    80004c1a:	e30fe0ef          	jal	8000324a <iupdate>
    80004c1e:	bf71                	j	80004bba <create+0xb6>
  ip->nlink = 0;
    80004c20:	040a1523          	sh	zero,74(s4)
  iupdate(ip);
    80004c24:	8552                	mv	a0,s4
    80004c26:	e24fe0ef          	jal	8000324a <iupdate>
  iunlockput(ip);
    80004c2a:	8552                	mv	a0,s4
    80004c2c:	8ddfe0ef          	jal	80003508 <iunlockput>
  iunlockput(dp);
    80004c30:	8526                	mv	a0,s1
    80004c32:	8d7fe0ef          	jal	80003508 <iunlockput>
  return 0;
...
0000000080004c3e <sys_dup>:
{
  ...
  filedup(f);
    ...
    80004c70:	c3eff0ef          	jal	800040ae <filedup>
  return fd;
...
0000000080004c8a <sys_read>:
{
  ...
  argaddr(1, &p);
    ...
    80004c98:	b61fd0ef          	jal	800027f8 <argaddr>
  argint(2, &n);
    80004c9c:	fe440593          	addi	a1,s0,-28
    80004ca0:	4509                	li	a0,2
    80004ca2:	b3bfd0ef          	jal	800027dc <argint>
  if(argfd(0, 0, &f) < 0)
  ...
  return fileread(f, p, n);
    ...
    80004cc6:	d4eff0ef          	jal	80004214 <fileread>
}
...
0000000080004cd2 <sys_write>:
{
  ...
  argaddr(1, &p);
    ...
    80004ce0:	b19fd0ef          	jal	800027f8 <argaddr>
  argint(2, &n);
    80004ce4:	fe440593          	addi	a1,s0,-28
    80004ce8:	4509                	li	a0,2
    80004cea:	af3fd0ef          	jal	800027dc <argint>
  if(argfd(0, 0, &f) < 0)
  ...
  return filewrite(f, p, n);
    ...
    80004d0e:	dc4ff0ef          	jal	800042d2 <filewrite>
}
...
0000000080004d1a <sys_close>:
{
  ...
  fileclose(f);
    ...
    80004d4c:	ba8ff0ef          	jal	800040f4 <fileclose>
  return 0;
...
0000000080004d5c <sys_fstat>:
{
  ...
  argaddr(1, &st);
    ...
    80004d6a:	a8ffd0ef          	jal	800027f8 <argaddr>
  if(argfd(0, 0, &f) < 0)
  ...
  return filestat(f, st);
    ...
    80004d8a:	c2cff0ef          	jal	800041b6 <filestat>
}
...
0000000080004d96 <sys_link>:
{
  ...
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    ...
    80004da8:	a6dfd0ef          	jal	80002814 <argstr>
    return -1;
  ...
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    ...
    80004dbc:	a59fd0ef          	jal	80002814 <argstr>
    return -1;
  ...
  begin_op();
    80004dc8:	f21fe0ef          	jal	80003ce8 <begin_op>
  if((ip = namei(old)) == 0){
    80004dcc:	ed040513          	addi	a0,s0,-304
    80004dd0:	d45fe0ef          	jal	80003b14 <namei>
    80004dd4:	84aa                	mv	s1,a0
    80004dd6:	c53d                	beqz	a0,80004e44 <sys_link+0xae>
  ilock(ip);
    80004dd8:	d26fe0ef          	jal	800032fe <ilock>
  if(ip->type == T_DIR){
  ...
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    ...
    80004e24:	c57fe0ef          	jal	80003a7a <dirlink>
    80004e28:	02054b63          	bltz	a0,80004e5e <sys_link+0xc8>
  iunlockput(dp);
    80004e2c:	854a                	mv	a0,s2
    80004e2e:	edafe0ef          	jal	80003508 <iunlockput>
  iput(ip);
    80004e32:	8526                	mv	a0,s1
    80004e34:	e4cfe0ef          	jal	80003480 <iput>
  end_op();
    80004e38:	f1bfe0ef          	jal	80003d52 <end_op>
  return 0;
    ...
    end_op();
    80004e44:	f0ffe0ef          	jal	80003d52 <end_op>
    return -1;
    ...
    80004e50:	eb8fe0ef          	jal	80003508 <iunlockput>
    end_op();
    80004e54:	efffe0ef          	jal	80003d52 <end_op>
    return -1;
    ...
    80004e60:	ea8fe0ef          	jal	80003508 <iunlockput>
  ilock(ip);
    80004e64:	8526                	mv	a0,s1
    80004e66:	c98fe0ef          	jal	800032fe <ilock>
  ip->nlink--;
  ...
  iupdate(ip);
    ...
    80004e76:	bd4fe0ef          	jal	8000324a <iupdate>
  iunlockput(ip);
    80004e7a:	8526                	mv	a0,s1
    80004e7c:	e8cfe0ef          	jal	80003508 <iunlockput>
  end_op();
    80004e80:	ed3fe0ef          	jal	80003d52 <end_op>
  return -1;
...
0000000080004e94 <sys_unlink>:
{
  ...
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    ...
    80004ed4:	9c5fe0ef          	jal	80003898 <namecmp>
    ...
    80004ee8:	9b1fe0ef          	jal	80003898 <namecmp>
  ...
  if((ip = dirlookup(dp, name, &off)) == 0)
    ...
    80004efc:	9b3fe0ef          	jal	800038ae <dirlookup>
    80004f00:	892a                	mv	s2,a0
    80004f02:	0e050863          	beqz	a0,80004ff2 <sys_unlink+0x15e>
  ilock(ip);
    80004f06:	bf8fe0ef          	jal	800032fe <ilock>
  if(ip->nlink < 1)
  ...
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    ...
    80004f36:	855fe0ef          	jal	8000378a <writei>
  ...
  iunlockput(dp);
    ...
    80004f4c:	dbcfe0ef          	jal	80003508 <iunlockput>
  ip->nlink--;
  ...
  iupdate(ip);
    ...
    80004f5c:	aeefe0ef          	jal	8000324a <iupdate>
  iunlockput(ip);
    80004f60:	854a                	mv	a0,s2
    80004f62:	da6fe0ef          	jal	80003508 <iunlockput>
  end_op();
    80004f66:	dedfe0ef          	jal	80003d52 <end_op>
  return 0;
    ...
    end_op();
    80004f72:	de1fe0ef          	jal	80003d52 <end_op>
    return -1;
  ...
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    ...
    80004fa8:	ee6fe0ef          	jal	8000368e <readi>
  ...
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    ...
    80004fec:	a5efe0ef          	jal	8000324a <iupdate>
    80004ff0:	bfa9                	j	80004f4a <sys_unlink+0xb6>
    80004ff2:	694e                	ld	s2,208(sp)
  iunlockput(dp);
    80004ff4:	8526                	mv	a0,s1
    80004ff6:	d12fe0ef          	jal	80003508 <iunlockput>
  end_op();
    80004ffa:	d59fe0ef          	jal	80003d52 <end_op>
  return -1;
    ...
    80005010:	cf8fe0ef          	jal	80003508 <iunlockput>
    goto bad;
...
sys_open(void)
{
    8000501a:	7131                	addi	sp,sp,-192
    8000501c:	fd06                	sd	ra,184(sp)
    8000501e:	f922                	sd	s0,176(sp)
    80005020:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;
  ...
  argint(1, &omode);
    80005022:	f4c40593          	addi	a1,s0,-180
    80005026:	4505                	li	a0,1
    80005028:	fb4fd0ef          	jal	800027dc <argint>
  if((n = argstr(0, path, MAXPATH)) < 0)
    8000502c:	08000613          	li	a2,128
    80005030:	f5040593          	addi	a1,s0,-176
    80005034:	4501                	li	a0,0
    80005036:	fdefd0ef          	jal	80002814 <argstr>
    8000503a:	87aa                	mv	a5,a0
    return -1;
    8000503c:	557d                	li	a0,-1
  if((n = argstr(0, path, MAXPATH)) < 0)
    8000503e:	0a07c263          	bltz	a5,800050e2 <sys_open+0xc8>
    80005042:	f526                	sd	s1,168(sp)

  begin_op();
    80005044:	ca5fe0ef          	jal	80003ce8 <begin_op>

  if(omode & O_CREATE){
    80005048:	f4c42783          	lw	a5,-180(s0)
    8000504c:	2007f793          	andi	a5,a5,512
    80005050:	c3d5                	beqz	a5,800050f4 <sys_open+0xda>
    ip = create(path, T_FILE, 0, 0);
    80005052:	4681                	li	a3,0
    80005054:	4601                	li	a2,0
    80005056:	4589                	li	a1,2
    80005058:	f5040513          	addi	a0,s0,-176
    8000505c:	aa9ff0ef          	jal	80004b04 <create>
    80005060:	84aa                	mv	s1,a0
    if(ip == 0){
    80005062:	c541                	beqz	a0,800050ea <sys_open+0xd0>
      end_op();
      return -1;
    }
  }
  ...
  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    ...
    80005068:	478d                	li	a5,3
    8000506a:	00f71763          	bne	a4,a5,80005078 <sys_open+0x5e>
    8000506e:	0464d703          	lhu	a4,70(s1)
    80005072:	47a5                	li	a5,9
    80005074:	0ae7ed63          	bltu	a5,a4,8000512e <sys_open+0x114>
    80005078:	f14a                	sd	s2,160(sp)
    iunlockput(ip);
    end_op();
    return -1;
  }
  ...
  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    8000507a:	fd7fe0ef          	jal	80004050 <filealloc>
    8000507e:	892a                	mv	s2,a0
    80005080:	c179                	beqz	a0,80005146 <sys_open+0x12c>
    80005082:	ed4e                	sd	s3,152(sp)
    80005084:	a43ff0ef          	jal	80004ac6 <fdalloc>
    80005088:	89aa                	mv	s3,a0
    8000508a:	0a054a63          	bltz	a0,8000513e <sys_open+0x124>
    iunlockput(ip);
    end_op();
    return -1;
  }
  ...
  if(ip->type == T_DEVICE){
    ...
    80005092:	478d                	li	a5,3
    80005094:	0cf70263          	beq	a4,a5,80005158 <sys_open+0x13e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80005098:	4789                	li	a5,2
    8000509a:	00f92023          	sw	a5,0(s2)
    f->off = 0;
    8000509e:	02092023          	sw	zero,32(s2)
  }
  f->ip = ip;
    800050a2:	00993c23          	sd	s1,24(s2)
  f->readable = !(omode & O_WRONLY);
    800050a6:	f4c42783          	lw	a5,-180(s0)
    800050aa:	0017c713          	xori	a4,a5,1
    800050ae:	8b05                	andi	a4,a4,1
    800050b0:	00e90423          	sb	a4,8(s2)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    800050b4:	0037f713          	andi	a4,a5,3
    800050b8:	00e03733          	snez	a4,a4
    800050bc:	00e904a3          	sb	a4,9(s2)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    800050c0:	4007f793          	andi	a5,a5,1024
    800050c4:	c791                	beqz	a5,800050d0 <sys_open+0xb6>
    800050c6:	04449703          	lh	a4,68(s1)
    800050ca:	4789                	li	a5,2
    800050cc:	08f70d63          	beq	a4,a5,80005166 <sys_open+0x14c>
    itrunc(ip);
  }

  iunlock(ip);
    800050d0:	8526                	mv	a0,s1
    800050d2:	adafe0ef          	jal	800033ac <iunlock>
  end_op();
    800050d6:	c7dfe0ef          	jal	80003d52 <end_op>

  return fd;
    800050da:	854e                	mv	a0,s3
    800050dc:	74aa                	ld	s1,168(sp)
    800050de:	790a                	ld	s2,160(sp)
    800050e0:	69ea                	ld	s3,152(sp)
}
    800050e2:	70ea                	ld	ra,184(sp)
    800050e4:	744a                	ld	s0,176(sp)
    800050e6:	6129                	addi	sp,sp,192
    800050e8:	8082                	ret
      end_op();
    800050ea:	c69fe0ef          	jal	80003d52 <end_op>
      return -1;
    800050ee:	557d                	li	a0,-1
    800050f0:	74aa                	ld	s1,168(sp)
    800050f2:	bfc5                	j	800050e2 <sys_open+0xc8>
    if((ip = namei(path)) == 0){
    800050f4:	f5040513          	addi	a0,s0,-176
    800050f8:	a1dfe0ef          	jal	80003b14 <namei>
    800050fc:	84aa                	mv	s1,a0
    800050fe:	c11d                	beqz	a0,80005124 <sys_open+0x10a>
    ilock(ip);
    80005100:	9fefe0ef          	jal	800032fe <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    80005104:	04449703          	lh	a4,68(s1)
    80005108:	4785                	li	a5,1
    8000510a:	f4f71de3          	bne	a4,a5,80005064 <sys_open+0x4a>
    8000510e:	f4c42783          	lw	a5,-180(s0)
    80005112:	d3bd                	beqz	a5,80005078 <sys_open+0x5e>
      iunlockput(ip);
    80005114:	8526                	mv	a0,s1
    80005116:	bf2fe0ef          	jal	80003508 <iunlockput>
      end_op();
    8000511a:	c39fe0ef          	jal	80003d52 <end_op>
      return -1;
    8000511e:	557d                	li	a0,-1
    80005120:	74aa                	ld	s1,168(sp)
    80005122:	b7c1                	j	800050e2 <sys_open+0xc8>
      end_op();
    80005124:	c2ffe0ef          	jal	80003d52 <end_op>
      return -1;
    80005128:	557d                	li	a0,-1
    8000512a:	74aa                	ld	s1,168(sp)
    8000512c:	bf5d                	j	800050e2 <sys_open+0xc8>
    iunlockput(ip);
    8000512e:	8526                	mv	a0,s1
    80005130:	bd8fe0ef          	jal	80003508 <iunlockput>
    end_op();
    80005134:	c1ffe0ef          	jal	80003d52 <end_op>
    return -1;
    80005138:	557d                	li	a0,-1
    8000513a:	74aa                	ld	s1,168(sp)
    8000513c:	b75d                	j	800050e2 <sys_open+0xc8>
      fileclose(f);
    8000513e:	854a                	mv	a0,s2
    80005140:	fb5fe0ef          	jal	800040f4 <fileclose>
    80005144:	69ea                	ld	s3,152(sp)
    iunlockput(ip);
    80005146:	8526                	mv	a0,s1
    80005148:	bc0fe0ef          	jal	80003508 <iunlockput>
    end_op();
    8000514c:	c07fe0ef          	jal	80003d52 <end_op>
    return -1;
    80005150:	557d                	li	a0,-1
    80005152:	74aa                	ld	s1,168(sp)
    80005154:	790a                	ld	s2,160(sp)
    80005156:	b771                	j	800050e2 <sys_open+0xc8>
    f->type = FD_DEVICE;
    80005158:	00f92023          	sw	a5,0(s2)
    f->major = ip->major;
    8000515c:	04649783          	lh	a5,70(s1)
    80005160:	02f91223          	sh	a5,36(s2)
    80005164:	bf3d                	j	800050a2 <sys_open+0x88>
    itrunc(ip);
    80005166:	8526                	mv	a0,s1
    80005168:	a84fe0ef          	jal	800033ec <itrunc>
    8000516c:	b795                	j	800050d0 <sys_open+0xb6>

000000008000516e <sys_mkdir>:

uint64
sys_mkdir(void)
{
    8000516e:	7175                	addi	sp,sp,-144
    80005170:	e506                	sd	ra,136(sp)
    80005172:	e122                	sd	s0,128(sp)
    80005174:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    80005176:	b73fe0ef          	jal	80003ce8 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    8000517a:	08000613          	li	a2,128
    8000517e:	f7040593          	addi	a1,s0,-144
    80005182:	4501                	li	a0,0
    80005184:	e90fd0ef          	jal	80002814 <argstr>
    80005188:	02054363          	bltz	a0,800051ae <sys_mkdir+0x40>
    8000518c:	4681                	li	a3,0
    8000518e:	4601                	li	a2,0
    80005190:	4585                	li	a1,1
    80005192:	f7040513          	addi	a0,s0,-144
    80005196:	96fff0ef          	jal	80004b04 <create>
    8000519a:	c911                	beqz	a0,800051ae <sys_mkdir+0x40>
    end_op();
    return -1;
  }
  iunlockput(ip);
    8000519c:	b6cfe0ef          	jal	80003508 <iunlockput>
  end_op();
    800051a0:	bb3fe0ef          	jal	80003d52 <end_op>
  return 0;
    800051a4:	4501                	li	a0,0
}
    800051a6:	60aa                	ld	ra,136(sp)
    800051a8:	640a                	ld	s0,128(sp)
    800051aa:	6149                	addi	sp,sp,144
    800051ac:	8082                	ret
    end_op();
    800051ae:	ba5fe0ef          	jal	80003d52 <end_op>
    return -1;
    800051b2:	557d                	li	a0,-1
    800051b4:	bfcd                	j	800051a6 <sys_mkdir+0x38>

00000000800051b6 <sys_mknod>:

uint64
sys_mknod(void)
{
    800051b6:	7135                	addi	sp,sp,-160
    800051b8:	ed06                	sd	ra,152(sp)
    800051ba:	e922                	sd	s0,144(sp)
    800051bc:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    800051be:	b2bfe0ef          	jal	80003ce8 <begin_op>
  argint(1, &major);
    800051c2:	f6c40593          	addi	a1,s0,-148
    800051c6:	4505                	li	a0,1
    800051c8:	e14fd0ef          	jal	800027dc <argint>
  argint(2, &minor);
    800051cc:	f6840593          	addi	a1,s0,-152
    800051d0:	4509                	li	a0,2
    800051d2:	e0afd0ef          	jal	800027dc <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    800051d6:	08000613          	li	a2,128
    800051da:	f7040593          	addi	a1,s0,-144
    800051de:	4501                	li	a0,0
    800051e0:	e34fd0ef          	jal	80002814 <argstr>
    800051e4:	02054563          	bltz	a0,8000520e <sys_mknod+0x58>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    800051e8:	f6841683          	lh	a3,-152(s0)
    800051ec:	f6c41603          	lh	a2,-148(s0)
    800051f0:	458d                	li	a1,3
    800051f2:	f7040513          	addi	a0,s0,-144
    800051f6:	90fff0ef          	jal	80004b04 <create>
  if((argstr(0, path, MAXPATH)) < 0 ||
    800051fa:	c911                	beqz	a0,8000520e <sys_mknod+0x58>
    end_op();
    return -1;
  }
  iunlockput(ip);
    800051fc:	b0cfe0ef          	jal	80003508 <iunlockput>
  end_op();
    80005200:	b53fe0ef          	jal	80003d52 <end_op>
  return 0;
    80005204:	4501                	li	a0,0
}
    80005206:	60ea                	ld	ra,152(sp)
    80005208:	644a                	ld	s0,144(sp)
    8000520a:	610d                	addi	sp,sp,160
    8000520c:	8082                	ret
    end_op();
    8000520e:	b45fe0ef          	jal	80003d52 <end_op>
    return -1;
    80005212:	557d                	li	a0,-1
    80005214:	bfcd                	j	80005206 <sys_mknod+0x50>

0000000080005216 <sys_chdir>:

uint64
sys_chdir(void)
{
    80005216:	7135                	addi	sp,sp,-160
    80005218:	ed06                	sd	ra,152(sp)
    8000521a:	e922                	sd	s0,144(sp)
    8000521c:	e14a                	sd	s2,128(sp)
    8000521e:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005220:	eaefc0ef          	jal	800018ce <myproc>
    80005224:	892a                	mv	s2,a0
  
  begin_op();
    80005226:	ac3fe0ef          	jal	80003ce8 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    8000522a:	08000613          	li	a2,128
    8000522e:	f6040593          	addi	a1,s0,-160
    80005232:	4501                	li	a0,0
    80005234:	de0fd0ef          	jal	80002814 <argstr>
    80005238:	04054363          	bltz	a0,8000527e <sys_chdir+0x68>
    8000523c:	e526                	sd	s1,136(sp)
    8000523e:	f6040513          	addi	a0,s0,-160
    80005242:	8d3fe0ef          	jal	80003b14 <namei>
    80005246:	84aa                	mv	s1,a0
    80005248:	c915                	beqz	a0,8000527c <sys_chdir+0x66>
    end_op();
    return -1;
  }
  ilock(ip);
    8000524a:	8b4fe0ef          	jal	800032fe <ilock>
  if(ip->type != T_DIR){
    8000524e:	04449703          	lh	a4,68(s1)
    80005252:	4785                	li	a5,1
    80005254:	02f71963          	bne	a4,a5,80005286 <sys_chdir+0x70>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    ...
    8000525a:	952fe0ef          	jal	800033ac <iunlock>
  iput(p->cwd);
    8000525e:	15093503          	ld	a0,336(s2)
    80005262:	a1efe0ef          	jal	80003480 <iput>
  end_op();
    80005266:	aedfe0ef          	jal	80003d52 <end_op>
  p->cwd = ip;
    8000526a:	14993823          	sd	s1,336(s2)
  return 0;
    8000526e:	4501                	li	a0,0
    80005270:	64aa                	ld	s1,136(sp)
}
    80005272:	60ea                	ld	ra,152(sp)
    80005274:	644a                	ld	s0,144(sp)
    80005276:	690a                	ld	s2,128(sp)
    80005278:	610d                	addi	sp,sp,160
    8000527a:	8082                	ret
    8000527c:	64aa                	ld	s1,136(sp)
    end_op();
    8000527e:	ad5fe0ef          	jal	80003d52 <end_op>
    return -1;
    80005282:	557d                	li	a0,-1
    80005284:	b7fd                	j	80005272 <sys_chdir+0x5c>
    iunlockput(ip);
    80005286:	8526                	mv	a0,s1
    80005288:	a80fe0ef          	jal	80003508 <iunlockput>
    end_op();
    8000528c:	ac7fe0ef          	jal	80003d52 <end_op>
    return -1;
    80005290:	557d                	li	a0,-1
    80005292:	64aa                	ld	s1,136(sp)
    80005294:	bff9                	j	80005272 <sys_chdir+0x5c>
...
sys_exec(void)
{
    80005296:	7121                	addi	sp,sp,-448
    80005298:	ff06                	sd	ra,440(sp)
    8000529a:	fb22                	sd	s0,432(sp)
    8000529c:	0380                	addi	s0,sp,448
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  argaddr(1, &uargv);
    8000529e:	e4840593          	addi	a1,s0,-440
    800052a2:	4505                	li	a0,1
    800052a4:	d54fd0ef          	jal	800027f8 <argaddr>
  if(argstr(0, path, MAXPATH) < 0) {
    800052a8:	08000613          	li	a2,128
    800052ac:	f5040593          	addi	a1,s0,-176
    800052b0:	4501                	li	a0,0
    800052b2:	d62fd0ef          	jal	80002814 <argstr>
    800052b6:	87aa                	mv	a5,a0
    return -1;
    800052b8:	557d                	li	a0,-1
  if(argstr(0, path, MAXPATH) < 0) {
    800052ba:	0c07c463          	bltz	a5,80005382 <sys_exec+0xec>
    800052be:	f726                	sd	s1,424(sp)
    800052c0:	f34a                	sd	s2,416(sp)
    800052c2:	ef4e                	sd	s3,408(sp)
    800052c4:	eb52                	sd	s4,400(sp)
  }
  memset(argv, 0, sizeof(argv));
    800052c6:	10000613          	li	a2,256
    800052ca:	4581                	li	a1,0
    800052cc:	e5040513          	addi	a0,s0,-432
    800052d0:	9d3fb0ef          	jal	80000ca2 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    800052d4:	e5040493          	addi	s1,s0,-432
  memset(argv, 0, sizeof(argv));
    800052d8:	89a6                	mv	s3,s1
    800052da:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    800052dc:	02000a13          	li	s4,32
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    800052e0:	00391513          	slli	a0,s2,0x3
    800052e4:	e4040593          	addi	a1,s0,-448
    800052e8:	e4843783          	ld	a5,-440(s0)
    800052ec:	953e                	add	a0,a0,a5
    800052ee:	c64fd0ef          	jal	80002752 <fetchaddr>
    800052f2:	02054663          	bltz	a0,8000531e <sys_exec+0x88>
      goto bad;
    }
    if(uarg == 0){
    800052f6:	e4043783          	ld	a5,-448(s0)
    800052fa:	c3a9                	beqz	a5,8000533c <sys_exec+0xa6>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    800052fc:	803fb0ef          	jal	80000afe <kalloc>
    80005300:	85aa                	mv	a1,a0
    80005302:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    80005306:	cd01                	beqz	a0,8000531e <sys_exec+0x88>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005308:	6605                	lui	a2,0x1
    8000530a:	e4043503          	ld	a0,-448(s0)
    8000530e:	c8efd0ef          	jal	8000279c <fetchstr>
    80005312:	00054663          	bltz	a0,8000531e <sys_exec+0x88>
    if(i >= NELEM(argv)){
    80005316:	0905                	addi	s2,s2,1
    80005318:	09a1                	addi	s3,s3,8
    8000531a:	fd4913e3          	bne	s2,s4,800052e0 <sys_exec+0x4a>
    kfree(argv[i]);
 ...
 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000531e:	f5040913          	addi	s2,s0,-176
    80005322:	6088                	ld	a0,0(s1)
    80005324:	c931                	beqz	a0,80005378 <sys_exec+0xe2>
    kfree(argv[i]);
    80005326:	ef6fb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000532a:	04a1                	addi	s1,s1,8
    8000532c:	ff249be3          	bne	s1,s2,80005322 <sys_exec+0x8c>
  return -1;
    ...
    8000533a:	a0a1                	j	80005382 <sys_exec+0xec>
      argv[i] = 0;
    8000533c:	0009079b          	sext.w	a5,s2
    80005340:	078e                	slli	a5,a5,0x3
    80005342:	fd078793          	addi	a5,a5,-48
    80005346:	97a2                	add	a5,a5,s0
    80005348:	e807b023          	sd	zero,-384(a5)
  int ret = kexec(path, argv);
    8000534c:	e5040593          	addi	a1,s0,-432
    80005350:	f5040513          	addi	a0,s0,-176
    80005354:	ba8ff0ef          	jal	800046fc <kexec>
    80005358:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000535a:	f5040993          	addi	s3,s0,-176
    8000535e:	6088                	ld	a0,0(s1)
    80005360:	c511                	beqz	a0,8000536c <sys_exec+0xd6>
    kfree(argv[i]);
    80005362:	ebafb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005366:	04a1                	addi	s1,s1,8
    80005368:	ff349be3          	bne	s1,s3,8000535e <sys_exec+0xc8>
  return ret;
    ...
    80005376:	a031                	j	80005382 <sys_exec+0xec>
  return -1;
    80005378:	557d                	li	a0,-1
    8000537a:	74ba                	ld	s1,424(sp)
    8000537c:	791a                	ld	s2,416(sp)
    8000537e:	69fa                	ld	s3,408(sp)
    80005380:	6a5a                	ld	s4,400(sp)
}
    80005382:	70fa                	ld	ra,440(sp)
    80005384:	745a                	ld	s0,432(sp)
    80005386:	6139                	addi	sp,sp,448
    80005388:	8082                	ret
...
sys_pipe(void)
{
  ...
  int fd0, fd1;
  struct proc *p = myproc();
    80005394:	d3afc0ef          	jal	800018ce <myproc>
    80005398:	84aa                	mv	s1,a0

  argaddr(0, &fdarray);
    8000539a:	fd840593          	addi	a1,s0,-40
    8000539e:	4501                	li	a0,0
    800053a0:	c58fd0ef          	jal	800027f8 <argaddr>
  if(pipealloc(&rf, &wf) < 0)
    800053a4:	fc840593          	addi	a1,s0,-56
    800053a8:	fd040513          	addi	a0,s0,-48
    800053ac:	852ff0ef          	jal	800043fe <pipealloc>
    return -1;
    800053b0:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    800053b2:	0a054463          	bltz	a0,8000545a <sys_pipe+0xd0>
  fd0 = -1;
    800053b6:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    800053ba:	fd043503          	ld	a0,-48(s0)
    800053be:	f08ff0ef          	jal	80004ac6 <fdalloc>
    800053c2:	fca42223          	sw	a0,-60(s0)
    800053c6:	08054163          	bltz	a0,80005448 <sys_pipe+0xbe>
    800053ca:	fc843503          	ld	a0,-56(s0)
    800053ce:	ef8ff0ef          	jal	80004ac6 <fdalloc>
    800053d2:	fca42023          	sw	a0,-64(s0)
    800053d6:	06054063          	bltz	a0,80005436 <sys_pipe+0xac>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  ...
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    ...
    800053dc:	fc440613          	addi	a2,s0,-60
    800053e0:	fd843583          	ld	a1,-40(s0)
    800053e4:	68a8                	ld	a0,80(s1)
    800053e6:	9fcfc0ef          	jal	800015e2 <copyout>
    800053ea:	00054e63          	bltz	a0,80005406 <sys_pipe+0x7c>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    800053ee:	4691                	li	a3,4
    800053f0:	fc040613          	addi	a2,s0,-64
    800053f4:	fd843583          	ld	a1,-40(s0)
    800053f8:	0591                	addi	a1,a1,4
    800053fa:	68a8                	ld	a0,80(s1)
    800053fc:	9e6fc0ef          	jal	800015e2 <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  ...
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005402:	04055c63          	bgez	a0,8000545a <sys_pipe+0xd0>
    p->ofile[fd0] = 0;
    80005406:	fc442783          	lw	a5,-60(s0)
    8000540a:	07e9                	addi	a5,a5,26
    ...
    8000541e:	0004b023          	sd	zero,0(s1)
    fileclose(rf);
    80005422:	fd043503          	ld	a0,-48(s0)
    80005426:	ccffe0ef          	jal	800040f4 <fileclose>
    fileclose(wf);
    8000542a:	fc843503          	ld	a0,-56(s0)
    8000542e:	cc7fe0ef          	jal	800040f4 <fileclose>
    return -1;
    80005432:	57fd                	li	a5,-1
    80005434:	a01d                	j	8000545a <sys_pipe+0xd0>
    if(fd0 >= 0)
    80005436:	fc442783          	lw	a5,-60(s0)
    8000543a:	0007c763          	bltz	a5,80005448 <sys_pipe+0xbe>
      p->ofile[fd0] = 0;
    8000543e:	07e9                	addi	a5,a5,26
    80005440:	078e                	slli	a5,a5,0x3
    80005442:	97a6                	add	a5,a5,s1
    80005444:	0007b023          	sd	zero,0(a5)
    fileclose(rf);
    80005448:	fd043503          	ld	a0,-48(s0)
    8000544c:	ca9fe0ef          	jal	800040f4 <fileclose>
    fileclose(wf);
    80005450:	fc843503          	ld	a0,-56(s0)
    80005454:	ca1fe0ef          	jal	800040f4 <fileclose>
    return -1;
    80005458:	57fd                	li	a5,-1
}
    8000545a:	853e                	mv	a0,a5
    8000545c:	70e2                	ld	ra,56(sp)
    8000545e:	7442                	ld	s0,48(sp)
    80005460:	74a2                	ld	s1,40(sp)
    80005462:	6121                	addi	sp,sp,64
    80005464:	8082                	ret
	...
...
kernelvec:
    ...
    80005494:	f9fe                	sd	t6,240(sp)
        ...
        call kerneltrap
    80005496:	9ccfd0ef          	jal	80002662 <kerneltrap>
...
free_desc(int i)
{
  ...
  wakeup(&disk.free[0]);
    ...
    800055b4:	971fc0ef          	jal	80001f24 <wakeup>
}
...
00000000800055d8 <virtio_disk_init>:
{
  ...
  *R(VIRTIO_MMIO_STATUS) = status;
  ...
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    ...
    80005658:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fddbe7>
  ...
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
  ...
  disk.desc = kalloc();
    ...
    800056ae:	38e48493          	addi	s1,s1,910 # 80020a38 <disk>
  ...
  if(!disk.desc || !disk.avail || !disk.used)
    ...
    800056cc:	37873703          	ld	a4,888(a4) # 80020a40 <disk+0x8>
  ...
  memset(disk.avail, 0, PGSIZE);
    800056e0:	0001b497          	auipc	s1,0x1b
    800056e4:	35848493          	addi	s1,s1,856 # 80020a38 <disk>
...
virtio_disk_rw(struct buf *b, int write)
{
  ...
  acquire(&disk.vdisk_lock);
    ...
    80005800:	36450513          	addi	a0,a0,868 # 80020b60 <disk+0x128>
  ...
  for(int i = 0; i < NUM; i++){
    8000580a:	44a1                	li	s1,8
      disk.free[i] = 0;
    8000580c:	0001bb17          	auipc	s6,0x1b
    80005810:	22cb0b13          	addi	s6,s6,556 # 80020a38 <disk>
  for(int i = 0; i < 3; i++){
  ...
  while(1){
    ...
    80005816:	0001bc17          	auipc	s8,0x1b
    8000581a:	34ac0c13          	addi	s8,s8,842 # 80020b60 <disk+0x128>
  ...
  for(int i = 0; i < NUM; i++){
    80005838:	0001b717          	auipc	a4,0x1b
    8000583c:	20070713          	addi	a4,a4,512 # 80020a38 <disk>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    80005870:	0001b517          	auipc	a0,0x1b
    80005874:	1e050513          	addi	a0,a0,480 # 80020a50 <disk+0x18>
    80005878:	e60fc0ef          	jal	80001ed8 <sleep>
  for(int i = 0; i < 3; i++){
  ...
  if(write)
    8000588c:	0001b797          	auipc	a5,0x1b
    80005890:	1ac78793          	addi	a5,a5,428 # 80020a38 <disk>
  ...
  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
  ...
  while(b->disk == 1) {
    ...
    80005962:	0001b917          	auipc	s2,0x1b
    80005966:	1fe90913          	addi	s2,s2,510 # 80020b60 <disk+0x128>
  while(b->disk == 1) {
    ...
    80005974:	d64fc0ef          	jal	80001ed8 <sleep>
  while(b->disk == 1) {
  ...
  disk.info[idx[0]].b = 0;
    ...
    8000598a:	0001b797          	auipc	a5,0x1b
    8000598e:	0ae78793          	addi	a5,a5,174 # 80020a38 <disk>
    80005992:	97ba                	add	a5,a5,a4
    80005994:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    80005998:	0001b997          	auipc	s3,0x1b
    8000599c:	0a098993          	addi	s3,s3,160 # 80020a38 <disk>
...
virtio_disk_intr()
{
  ...
  acquire(&disk.vdisk_lock);
    800059ec:	0001b497          	auipc	s1,0x1b
    800059f0:	04c48493          	addi	s1,s1,76 # 80020a38 <disk>
    800059f4:	0001b517          	auipc	a0,0x1b
    800059f8:	16c50513          	addi	a0,a0,364 # 80020b60 <disk+0x128>
  ...
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
  ...
  while(disk.used_idx != disk.used->idx){
    ...
    wakeup(b);
    80005a4c:	cd8fc0ef          	jal	80001f24 <wakeup>
  ...
  release(&disk.vdisk_lock);
    80005a68:	0001b517          	auipc	a0,0x1b
    80005a6c:	0f850513          	addi	a0,a0,248 # 80020b60 <disk+0x128>
